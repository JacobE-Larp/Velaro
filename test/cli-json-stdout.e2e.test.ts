...
describe("cli json stdout contract", () => {
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        const legacyDir = path.join(tempHome, ".vilaro");
        await fs.mkdir(legacyDir, { recursive: true });
        await fs.writeFile(path.join(legacyDir, "vilaro.json"), "{}", "utf8");
        ...
        delete env.VILARO_HOME;
        delete env.VILARO_STATE_DIR;
        delete env.VILARO_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "vilaro.mjs");
