...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `vilaro-e2e-${name}-`));
  const configDir = path.join(homeDir, ".vilaro");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          VILARO_SKIP_GMAIL_WATCHER: "1",
          VILARO_SKIP_BROWSER_CONTROL_SERVER: "1",
          VILARO_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
