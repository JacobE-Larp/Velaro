...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked VILARO_HOME.
  delete process.env.VILARO_HOME;
  process.env.VILARO_STATE_DIR = path.join(base, ".vilaro");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "vilaro-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".vilaro", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
