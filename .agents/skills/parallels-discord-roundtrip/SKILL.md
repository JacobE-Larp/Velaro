...
```bash
export VILARO_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.vilaro/vilaro.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env VILARO_PARALLELS_DISCORD_TOKEN \
...
- Use the `vilaro` wrapper for guest `message send/read`; `node vilaro.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/vilaro-parallels-smoke.*`
...
- host inbound nonce appears in `vilaro message read` output
