...
Deploy Vilaro on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/vilaro-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/vilaro` — Control UI

## What you get

- Hosted Vilaro Gateway + Control UI
...
- `VILARO_STATE_DIR=/data/.vilaro` (recommended)
- `VILARO_WORKSPACE_DIR=/data/workspace` (recommended)
- `VILARO_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Vilaro state + workspace so you can migrate to another host without losing config or memory.
