...
Deploy Vilaro on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/vilaro/vilaro)
...
repository configures everything needed to run Vilaro:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: VILARO_STATE_DIR
        value: /data/.vilaro
      - key: VILARO_WORKSPACE_DIR
        value: /data/workspace
      - key: VILARO_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/vilaro`.
...
If you use the original Vilaro repository, Render will not auto-deploy your Vilaro. To update it, run a manual Blueprint sync from the dashboard.
...
For Vilaro, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Vilaro host.
