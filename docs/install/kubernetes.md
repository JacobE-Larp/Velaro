...
Edit `vilaro.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
VILARO_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
└── manifests/
    ...
    ├── configmap.yaml          # vilaro.json + AGENTS.md
