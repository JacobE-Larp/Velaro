...
<AccordionGroup>
  <Accordion title="Installer script" icon="rocket" defaultOpen>
    ...
    <Tabs>
      <Tab title="macOS / Linux / WSL2">
        ```bash
        curl -fsSL https://vilaro.ai/install.sh | bash -s -- --no-onboard
        ```
      </Tab>
      <Tab title="Windows (PowerShell)">
        ```powershell
...
- `VILARO_HOME` for home-directory based internal paths
- `VILARO_STATE_DIR` for mutable state location
- `VILARO_CONFIG_PATH` for config file location
