...
- `/usage cost` in chats: local cost summary aggregated from Vilaro session logs.
- CLI: `vilaro status --usage` prints a full per-provider breakdown.
- CLI: `vilaro channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
