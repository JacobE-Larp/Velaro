...
- `/usage cost` in chats: local cost summary aggregated from Velaro session logs.
- CLI: `velaro status --usage` prints a full per-provider breakdown.
- CLI: `velaro channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
