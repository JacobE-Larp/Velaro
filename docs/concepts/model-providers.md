...
- CLI helpers: `velaro onboard`, `velaro models list`, `velaro models set <provider/model>`.
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  Velaro merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while Velaro keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `velaro onboard`, `velaro models auth`, and headless setup
...
That covers providers that still fit Velaro's normal transports. A provider
...
Velaro ships with the pi‑ai catalog. These providers require **no**
...
- CLI: `velaro onboard --auth-choice openai-api-key`
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in Velaro because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- CLI: `velaro onboard --auth-choice token` (paste setup-token) or `velaro models auth paste-token --provider anthropic`
- Direct API-key models support the shared `/fast` toggle and `params.fastMode`; Velaro maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- CLI: `velaro onboard --auth-choice openai-codex` or `velaro models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like Velaro.
...
- CLI: `velaro onboard --auth-choice opencode-zen` or `velaro onboard --auth-choice opencode-go`
...
- Compatibility: legacy Velaro config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `velaro onboard --auth-choice gemini-api-key`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in Velaro is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Enable: `velaro plugins enable google`
  - Login: `velaro models auth login --provider google-gemini-cli --set-default`
...
- CLI: `velaro onboard --auth-choice zai-api-key`
...
- CLI: `velaro onboard --auth-choice ai-gateway-api-key`
...
- CLI: `velaro onboard --kilocode-api-key <key>`
...
- CLI: `velaro onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `velaro onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
```bash
velaro models auth login --provider qwen-portal --set-default
```
...
- CLI: `velaro onboard --auth-choice volcengine-api-key`
...
- CLI: `velaro onboard --auth-choice byteplus-api-key`
...
- CLI: `velaro onboard --auth-choice synthetic-api-key`
...
`velaro onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, Velaro defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), Velaro forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- If `baseUrl` is empty/omitted, Velaro keeps the default OpenAI behavior (which resolves to `api.openai.com`).
...
velaro models set opencode/claude-opus-4-6
velaro models list
```
