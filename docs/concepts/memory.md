...
`~/.vilaro/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.vilaro/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.vilaro/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Session logs live on disk (`~/.vilaro/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
