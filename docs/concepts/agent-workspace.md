...
This is separate from `~/.vilaro/`, which stores config, credentials, and
sessions.
...
inside a sandbox workspace under `~/.vilaro/sandboxes`, not your host workspace.

## Default location

- Default: `~/.vilaro/workspace`
- If `VILARO_PROFILE` is set and not `"default"`, the default becomes
  `~/.vilaro/workspace-<profile>`.
- Override in `~/.vilaro/vilaro.json`:
...
These live under `~/.vilaro/` and should NOT be committed to the workspace repo:

- `~/.vilaro/vilaro.json` (config)
- `~/.vilaro/credentials/` (OAuth tokens, API keys)
- `~/.vilaro/agents/<agentId>/sessions/` (session transcripts + metadata)
- `~/.vilaro/skills/` (managed skills)
...
- Anything under `~/.vilaro/`.
- Raw dumps of chats or sensitive attachments.

If you must store sensitive references, use placeholders and keep the real
secret elsewhere (password manager, environment variables, or `~/.vilaro/`).
