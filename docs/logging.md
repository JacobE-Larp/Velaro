...
You can override this in `~/.vilaro/vilaro.json`:
...
All logging configuration lives under `logging` in `~/.vilaro/vilaro.json`.
...
```
VILARO_DIAGNOSTICS=telegram.http,telegram.payload
```
