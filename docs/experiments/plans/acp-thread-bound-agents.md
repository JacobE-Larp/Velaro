...
This plan defines how Vilaro should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Vilaro core owns ACP control plane concerns
...
Vilaro should not reimplement ACP transport internals in core.
Vilaro should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Vilaro, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Vilaro state dir
...
- ACP event stream is normalized to Vilaro reply chunks
...
For the first production backend (`extensions/acpx`), Vilaro and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Vilaro `sessionKey`)
...
- Vilaro consumes ndjson events from `acpx --format json --json-strict`
