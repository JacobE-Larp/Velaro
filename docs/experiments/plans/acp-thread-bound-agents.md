...
This plan defines how Velaro should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Velaro core owns ACP control plane concerns
...
Velaro should not reimplement ACP transport internals in core.
Velaro should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Velaro, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Velaro state dir
...
- ACP event stream is normalized to Velaro reply chunks
...
For the first production backend (`extensions/acpx`), Velaro and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Velaro `sessionKey`)
...
- Velaro consumes ndjson events from `acpx --format json --json-strict`
