...
read_when:
  - Running Velaro behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Velaro
...
- You run Velaro behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Velaro checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Velaro extracts the user identity from the configured header
...
- Velaro can stay on loopback HTTP behind the proxy.
...
If Velaro itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://vilaro.example.com
    to: http://vilaro-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`velaro security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Velaro config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `velaro security audit` and review findings
