...
read_when:
  - Running Vilaro behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Vilaro
...
- You run Vilaro behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Vilaro checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Vilaro extracts the user identity from the configured header
...
- Vilaro can stay on loopback HTTP behind the proxy.
...
If Vilaro itself serves HTTPS directly (no TLS-terminating proxy), set:
...
`vilaro security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Vilaro config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `vilaro security audit` and review findings
