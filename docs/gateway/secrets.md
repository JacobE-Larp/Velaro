...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `VILARO_GATEWAY_TOKEN` (or `VILARO_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `vilaro.json` refs)
...
- plaintext values at rest (`vilaro.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `vilaro.json` refs)
...
- lets you select supported secret-bearing fields in `vilaro.json` plus `auth-profiles.json` for one agent scope
