...
Velaro supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `velaro gateway`).
...
API keys for daemon use: `velaro onboard`.
...
```bash
velaro models auth paste-token --provider anthropic
velaro models auth paste-token --provider openrouter
```
...
- Velaro retries with the next key only for rate-limit errors (for example
...
Run `velaro models status` to confirm which profile is expiring. If the profile
