...
<Tabs>
  ...
  <Tab title="Direct edit">
    Edit `~/.vilaro/vilaro.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `vilaro.json`.
    ...
    - `VILARO_APNS_RELAY_BASE_URL` and `VILARO_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `VILARO_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.vilaro/vilaro.json` and applies changes automatically — no manual restart needed for most settings.
...
Env var equivalent: `VILARO_LOAD_SHELL_ENV=1`
</Accordion>
