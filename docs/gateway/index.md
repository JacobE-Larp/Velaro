...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `VILARO_CONFIG_PATH` when set).
...
- Auth is required by default (`gateway.auth.token` / `gateway.auth.password`, or `VILARO_GATEWAY_TOKEN` / `VILARO_GATEWAY_PASSWORD`).

### Port and bind precedence

| Setting      | Resolution order                                            |
| ------------ | ----------------------------------------------------------- |
| Gateway port | `--port` → `VILARO_GATEWAY_PORT` → `gateway.port` → `18789` |
...
- Unique `gateway.port`
- Unique `VILARO_CONFIG_PATH`
- Unique `VILARO_STATE_DIR`
