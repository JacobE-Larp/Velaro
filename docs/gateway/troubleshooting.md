...
- `vilaro gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `vilaro doctor` reports no blocking config/service issues.
- `vilaro channels status --probe` shows connected/ready channels.
...
vilaro config get agents.defaults.models
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `vilaro config get gateway.auth.token` then paste into Control UI settings.                            |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `vilaro devices list` then `vilaro devices approve <requestId>`.                                                                            |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `vilaro configure`). If you are running Vilaro via Podman using the dedicated `vilaro` user, the config lives at `~vilaro/.vilaro/vilaro.json`.
...
vilaro browser start --browser-profile vilaro
