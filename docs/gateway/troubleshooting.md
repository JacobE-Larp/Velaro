...
- `velaro gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `velaro doctor` reports no blocking config/service issues.
- `velaro channels status --probe` shows connected/ready channels.
...
velaro config get agents.defaults.models
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `velaro config get gateway.auth.token` then paste into Control UI settings.                            |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `velaro devices list` then `velaro devices approve <requestId>`.                                                                            |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `velaro configure`). If you are running Velaro via Podman using the dedicated `velaro` user, the config lives at `~velaro/.velaro/vilaro.json`.
...
velaro browser start --browser-profile velaro
...
```bash
velaro gateway status
velaro config get gateway.mode
velaro config get gateway.remote.url
velaro config get gateway.auth.mode
```
...
```bash
velaro config get gateway.bind
