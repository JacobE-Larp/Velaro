...
4. Claim is not listed in [Out of Scope](https://github.com/vilaro/vilaro/blob/main/SECURITY.md#out-of-scope).
...
- **Pairing allowlists**:
  - `~/.vilaro/credentials/<channel>-allowFrom.json` (default account)
  - `~/.vilaro/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.vilaro/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.vilaro/secrets.json`
- **Legacy OAuth import**: `~/.vilaro/credentials/oauth.json`
...
- “Paste the full contents of ~/.vilaro or your logs.”
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `VILARO_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__vilaro__/canvas/` and `/__vilaro__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_vilaro-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `VILARO_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
paths, set `VILARO_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `VILARO_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `VILARO_GATEWAY_PASSWORD`).
...
Assume anything under `~/.vilaro/` (or `$VILARO_STATE_DIR/`) may contain secrets or private data:

- `vilaro.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.vilaro`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.vilaro/sandboxes`
...
1. Rotate Gateway auth (`gateway.auth.token` / `VILARO_GATEWAY_PASSWORD`) and restart.
