...
- `VILARO_CONFIG_PATH` — per-instance config file
- `VILARO_STATE_DIR` — per-instance sessions, creds, caches
...
Profiles auto-scope `VILARO_STATE_DIR` + `VILARO_CONFIG_PATH` and suffix service names.
...
Base port = `gateway.port` (or `VILARO_GATEWAY_PORT` / `--port`).
