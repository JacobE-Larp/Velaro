...
- Rewrite `~/.vilaro/vilaro.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.vilaro/sessions/` to `~/.vilaro/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.vilaro/agent/` to `~/.vilaro/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.vilaro/credentials/*.json` (except `oauth.json`)
  - to `~/.vilaro/credentials/whatsapp/<accountId>/...` (default account id: `default`)
...
Doctor also checks the cron job store (`~/.vilaro/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.vilaro` folders exist across
  home directories or when `VILARO_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.vilaro/vilaro.json` is
