...
- During heartbeat runs, Velaro treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `velaro sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Velaro skips the heartbeat run entirely (no model call).
...
```bash
velaro system event --text "Check for urgent follow-ups" --mode now
```
