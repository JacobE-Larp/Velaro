...
- During heartbeat runs, Vilaro treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `vilaro sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Vilaro skips the heartbeat run entirely (no model call).
...
```bash
vilaro system event --text "Check for urgent follow-ups" --mode now
```
