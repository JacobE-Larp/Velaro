...
Vilaro can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Vilaro requires a shared password.
...
- `token` (default when `VILARO_GATEWAY_TOKEN` is set)
- `password` (shared secret via `VILARO_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
Control UI/WebSocket auth can use Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Vilaro verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Vilaro only treats a request as Serve when it arrives from loopback with
...
Prefer `VILARO_GATEWAY_PASSWORD` over committing a password to disk.
...
- Set `gateway.tailscale.resetOnExit` if you want Vilaro to undo `tailscale serve`
