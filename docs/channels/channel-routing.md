...
Vilaro routes replies **back to the channel where a message came from**. The
...
Vilaro infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Vilaro still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Vilaro would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.vilaro/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.vilaro`):

- `~/.vilaro/agents/<agentId>/sessions/sessions.json`
