---
summary: "Mattermost bot setup and Vilaro config"
read_when:
...
```bash
vilaro plugins install @vilaro/mattermost
```

Local checkout (when running from a git repo):

```bash
vilaro plugins install ./extensions/mattermost
```
...
4. Configure Vilaro and start the gateway.
...
Native slash commands are opt-in. When enabled, Vilaro registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Vilaro derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Vilaro.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Vilaro.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Vilaro, not `404`.
...
- Approve via:
  - `vilaro pairing list mattermost`
  - `vilaro pairing approve mattermost <CODE>`
...
Use these target formats with `vilaro message send` or cron/webhooks:
...
Vilaro resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Vilaro sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, Vilaro derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Vilaro run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="vilaro-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"vilaro-mattermost-interactions",
...
`vilaro message send` and cron/webhook deliveries.
