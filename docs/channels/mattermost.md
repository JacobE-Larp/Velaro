---
summary: "Mattermost bot setup and Velaro config"
read_when:
...
```bash
velaro plugins install @vilaro/mattermost
```

Local checkout (when running from a git repo):

```bash
velaro plugins install ./extensions/mattermost
```
...
4. Configure Velaro and start the gateway.
...
Native slash commands are opt-in. When enabled, Velaro registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Velaro derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Velaro.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Velaro.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Velaro, not `404`.
...
- Approve via:
  - `velaro pairing list mattermost`
  - `velaro pairing approve mattermost <CODE>`
...
Use these target formats with `velaro message send` or cron/webhooks:
...
Velaro resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Velaro sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, Velaro derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Velaro run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="velaro-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"velaro-mattermost-interactions",
...
`velaro message send` and cron/webhook deliveries.
