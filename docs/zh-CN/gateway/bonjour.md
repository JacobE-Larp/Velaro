...
你的发现域名中浏览 `_vilaro-gw._tcp`，无需多播。
...
- 在 `~/.vilaro/vilaro.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway 网关（或重启 macOS 菜单栏应用）。

## 什么在广播

只有 Gateway 网关广播 `_vilaro-gw._tcp`。

## 服务类型

- `_vilaro-gw._tcp` — Gateway 网关传输信标（被 macOS/iOS/Android 节点使用）。
...
iOS 节点使用 `NWBrowser` 来发现 `_vilaro-gw._tcp`。
...
- `VILARO_DISABLE_BONJOUR=1` 禁用广播（旧版：`VILARO_DISABLE_BONJOUR`）。
- `~/.vilaro/vilaro.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `VILARO_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`VILARO_SSH_PORT`）。
- `VILARO_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`VILARO_TAILNET_DNS`）。
- `VILARO_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`VILARO_CLI_PATH`）。
