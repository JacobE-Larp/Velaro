---
read_when:
  - 在 DigitalOcean 上设置 Vilaro
  - 寻找适合 Vilaro 的低价 VPS 托管
summary: 在 DigitalOcean 上运行 Vilaro（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Vilaro

## 目标

在 DigitalOcean 上以 **每月 6 美元**（或预留定价时每月 4 美元）运行一个持久化的 Vilaro Gateway 网关。
