...
- 你可以在 Gateway 网关机器上运行 CLI（`velaro`）（或通过 SSH）。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `velaro.internal.`）并发布 `_vilaro-gw._tcp` 记录。
...
```bash
velaro nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__vilaro__/canvas/"}'
```
