...
macOS 也可以在**节点模式**下运行：菜单栏应用连接到 Gateway 网关的 WS 服务器，并将其本地 canvas/camera 命令作为节点暴露（因此 `vilaro nodes …` 可以针对这台 Mac 工作）。
...
- `node.pair.*`（CLI：`vilaro nodes pending/approve/reject`）是一个单独的 Gateway 网关拥有的
...
- `vilaro node run` 读取 `VILARO_GATEWAY_TOKEN` 进行认证。
...
- 在 `vilaro node run` / `vilaro node install` 上使用 `--display-name`（持久化在节点上的 `~/.vilaro/node.json` 中）。
- `vilaro nodes rename --node <id|name|ip> --name "Build Node"`（Gateway 网关覆盖）。
...
vilaro nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
...
Vilaro 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
...
- macOS 菜单栏应用作为节点连接到 Gateway 网关 WS 服务器（因此 `vilaro nodes …` 可以针对这台 Mac 工作）。
