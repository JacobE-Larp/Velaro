...
定时任务默认持久化存储在 Gateway网关主机的 `~/.vilaro/cron/jobs.json` 中。Gateway网关将文件加载到内存中，并在更改时写回，因此仅在 Gateway网关停止时手动编辑才是安全的。请优先使用 `vilaro cron add/edit` 或定时任务工具调用 API 进行更改。
...
- `cron.run`（强制或到期）、`cron.runs`
  如需不创建任务直接发送系统事件，请使用 [`vilaro system event`](/cli/system)。
