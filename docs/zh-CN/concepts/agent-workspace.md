...
这与存储配置、凭证和会话的 `~/.vilaro/` 是分开的。

**重要：**工作区是**默认 cwd**，而不是硬性沙箱。工具会相对于工作区解析相对路径，但除非启用沙箱隔离，否则绝对路径仍然可以访问主机上的其他位置。如果你需要隔离，请使用 [`agents.defaults.sandbox`](/gateway/sandboxing)（和/或按智能体配置的沙箱）。启用沙箱隔离后，如果 `workspaceAccess` 不是 `"rw"`，工具会在 `~/.vilaro/sandboxes` 下的沙箱工作区中运行，而不是在你的主机工作区中运行。

## 默认位置

- 默认值：`~/.vilaro/workspace`
- 如果设置了 `VILARO_PROFILE` 且其值不是 `"default"`，默认值将变为
  `~/.vilaro/workspace-<profile>`。
- 在 `~/.vilaro/vilaro.json` 中覆盖：
...
这些内容位于 `~/.vilaro/` 下，**不应**提交到工作区仓库：

- `~/.vilaro/vilaro.json`（配置）
- `~/.vilaro/credentials/`（OAuth 令牌、API 密钥）
- `~/.vilaro/agents/<agentId>/sessions/`（会话记录和元数据）
- `~/.vilaro/skills/`（托管 Skills）
...
如果你必须存储敏感引用，请使用占位符，并将真实密钥保存在其他地方（密码管理器、环境变量或 `~/.vilaro/`）。
