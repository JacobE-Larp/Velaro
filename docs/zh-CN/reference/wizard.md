...
<Steps>
  <Step title="现有配置检测">
    - 如果 `~/.vilaro/vilaro.json` 存在，请选择 **Keep / Modify / Reset**。
  ...
  <Step title="模型/认证">
    ...
    - OAuth 凭证保存在 `~/.vilaro/credentials/oauth.json`；auth-profile 保存在 `~/.vilaro/agents/<agentId>/agent/auth-profiles.json`（API key + OAuth）。
    - 更多细节： [/concepts/oauth](/concepts/oauth)
    <Note>
    无头/服务器提示：在有浏览器的机器上完成 OAuth，然后将
    `~/.vilaro/credentials/oauth.json`（或 `$VILARO_STATE_DIR/credentials/oauth.json`）复制到
    Gateway 网关主机上。
    </Note>
  </Step>
  <Step title="工作区">
    - 默认为 `~/.vilaro/workspace`（可配置）。
    - 为工作区植入智能体引导仪式所需的文件。
...
- 下载适合的发布资源。
- 将其存储到 `~/.vilaro/tools/signal-cli/<version>/` 下。
