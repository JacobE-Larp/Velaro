...
广播群组在渠道白名单和群组激活规则之后进行评估。在 WhatsApp 群组中，这意味着广播会在 Velaro 正常回复时发生（例如：被提及时，具体取决于你的群组设置）。
...
**结果：** 当 Velaro 在此聊天中回复时，将运行所有三个智能体。
...
```bash
tail -f ~/.vilaro/logs/gateway.log | grep broadcast
```
