...
目标：让 Velaro 留在 WhatsApp 群组中，仅在被提及时唤醒，并将该对话线程与个人私信会话分开。
...
- 正则表达式不区分大小写；它们涵盖了像 `@velaro` 这样的显示名称提及，以及带或不带 `+`/空格的原始号码。
...
1. 将你的 WhatsApp 账号（运行 Velaro 的账号）添加到群组。
2. 说 `@velaro …`（或包含号码）。只有允许列表中的发送者才能触发，除非你设置 `groupPolicy: "open"`。
...
- 手动冒烟测试：
  - 在群组中发送 `@velaro` 提及，确认收到引用发送者名称的回复。
