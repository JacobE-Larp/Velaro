...
- 会话文件：`~/.vilaro/agents/<agentId>/sessions/{{SessionId}}.jsonl`
- 会话元数据（token 使用量、最后路由等）：`~/.vilaro/agents/<agentId>/sessions/sessions.json`（旧版：`~/.vilaro/sessions/sessions.json`）
