...
- OAuth 凭证：`~/.vilaro/credentials/oauth.json`
- Auth profile（API 密钥 + OAuth）：`~/.vilaro/agents/<agentId>/agent/auth-profiles.json`
...
`~/.vilaro/credentials/oauth.json`（或 `$VILARO_STATE_DIR/credentials/oauth.json`）
到 Gateway 网关主机。
</Note>
...
- 下载适当的发布资源
- 将其存储在 `~/.vilaro/tools/signal-cli/<version>/`
