...
read_when:
  - 设计 macOS 新手引导助手
  - 实现认证或身份设置
summary: Vilaro 的首次运行新手引导流程（macOS 应用）
title: 新手引导
x-i18n:
...
应用可以通过 npm/pnpm 安装全局 `vilaro` CLI，以便终端工作流和 launchd 任务开箱即用。
...
在首次智能体运行时，Vilaro 会引导一个工作区（默认 `~/.vilaro/workspace`）：
...
```bash
vilaro webhooks gmail setup --account you@gmail.com
```
