...
设置向导是在 macOS、
Linux 或 Windows（通过 WSL2；强烈推荐）上设置 Vilaro 的**推荐**方式。
...
`vilaro dashboard` 并在浏览器中聊天。文档：[Dashboard](/web/dashboard)。
</Info>
...
`vilaro configure --section web` 进行配置。文档：[Web 工具](/tools/web)。
</Tip>
...
如果配置无效或包含旧版键，向导会先要求你运行 `vilaro doctor`。
</Note>
...
使用 `vilaro agents add <name>` 创建一个单独的智能体，它拥有自己的工作区、
...
- CLI 命令参考：[`vilaro onboard`](/cli/onboard)
