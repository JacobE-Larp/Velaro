...
- **个性化设置存放在仓库之外：** `~/.vilaro/workspace`（工作区）+ `~/.vilaro/vilaro.json`（配置）。
...
- **配置：** `~/.vilaro/vilaro.json`（JSON/JSON5 格式）
- **工作区：** `~/.vilaro/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
- **配对允许列表**：`~/.vilaro/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.vilaro/agents/<agentId>/agent/auth-profiles.json`
