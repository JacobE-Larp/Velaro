...
- 要获取完整的提供商使用量分解，使用 `vilaro status --usage`。
- `/allowlist add|remove` 需要 `commands.config=true` 并遵循渠道 `configWrites`。
- `/usage` 控制每响应使用量页脚；`/usage cost` 从 Vilaro 会话日志打印本地成本摘要。
...
- **Slack：** `channels.slack.slashCommand` 仍然支持单个 `/vilaro` 风格的命令。如果你启用 `commands.native`，你必须为每个内置命令创建一个 Slack 斜杠命令（与 `/help` 相同的名称）。Slack 的命令参数菜单以临时 Block Kit 按钮形式发送。
