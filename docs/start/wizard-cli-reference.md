...
- OAuth credentials: `~/.vilaro/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.vilaro/agents/<agentId>/agent/auth-profiles.json`
...
`~/.vilaro/credentials/oauth.json` (or `$VILARO_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
