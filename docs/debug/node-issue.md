...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Vilaro
...
Running Vilaro via Node with `tsx` fails at startup with:

```
[vilaro] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `vilaro status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch vilaro.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node vilaro.mjs status` works on Node 25.
