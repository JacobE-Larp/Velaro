...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Velaro
...
Running Velaro via Node with `tsx` fails at startup with:

```
[velaro] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `velaro status` and `gateway:watch` worked.
