...
read_when:
  ...
  - Wiring external systems into Velaro
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Velaro config directory (typically `~/.vilaro/hooks/transforms`).
...
- `velaro webhooks gmail setup` writes `hooks.gmail` config for `velaro webhooks gmail run`.
