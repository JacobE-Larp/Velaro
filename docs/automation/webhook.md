...
read_when:
  ...
  - Wiring external systems into Vilaro
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Vilaro config directory (typically `~/.vilaro/hooks/transforms`).
...
- `vilaro webhooks gmail setup` writes `hooks.gmail` config for `vilaro webhooks gmail run`.
