...
- Jobs persist under `~/.vilaro/cron/` so restarts don’t lose schedules.
...
- Job store: `~/.vilaro/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.vilaro/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- Check cron is enabled: `cron.enabled` and `VILARO_SKIP_CRON`.
