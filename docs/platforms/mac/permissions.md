...
- Same path: run the app from a fixed location (for Vilaro, `dist/Vilaro.app`).
...
```bash
sudo tccutil reset Accessibility ai.vilaro.mac
sudo tccutil reset ScreenCapture ai.vilaro.mac
...
Workaround: move files into the Vilaro workspace (`~/.vilaro/workspace`) if you want to avoid per-folder grants.
