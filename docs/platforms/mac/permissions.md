...
- Same path: run the app from a fixed location (for Velaro, `dist/Velaro.app`).
...
```bash
sudo tccutil reset Accessibility ai.velaro.mac
sudo tccutil reset ScreenCapture ai.velaro.mac
...
Workaround: move files into the Velaro workspace (`~/.vilaro/workspace`) if you want to avoid per-folder grants.
