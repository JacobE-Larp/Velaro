...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `vilaro.json` refs)
...
- `configure` targets secret-bearing fields in `vilaro.json` plus `auth-profiles.json` for the selected agent scope.
...
- `vilaro.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.vilaro/.env` known secret keys whose values were migrated
