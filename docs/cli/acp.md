...
```bash
env VILARO_HIDE_BANNER=1 VILARO_SUPPRESS_NOTES=1 node vilaro.mjs acp ...
```
...
- Prefer `--token-file`/`--password-file` or environment variables (`VILARO_GATEWAY_TOKEN`, `VILARO_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`VILARO_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
