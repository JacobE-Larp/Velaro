...
Vilaro can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Vilaro can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Vilaro defaults
...
TTS config lives under `messages.tts` in `vilaro.json`.
...
- If `provider` is **unset**, Vilaro prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.vilaro/settings/tts.json`, override with `VILARO_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Vilaro retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Vilaro registers
