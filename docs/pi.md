...
summary: "Architecture of Vilaro's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Vilaro
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Vilaro integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Vilaro uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Vilaro directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Vilaro's local TUI mode)                                               |
...
├── pi-tools.ts                    # createVilaroCodingTools()
...
├── vilaro-tools.ts              # Vilaro-specific tools
...
Image injection is prompt-local: Vilaro loads image refs from the current prompt and
...
2. **Custom Replacements**: Vilaro replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Vilaro Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
Vilaro wraps this with `guardSessionManager()` for tool result safety.
...
Vilaro maintains an auth profile store with multiple API keys per provider:
...
Vilaro loads custom pi extensions for specialized behavior:
...
Vilaro also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Vilaro Embedded                                                                            |
| --------------- | ----------------------- | ------------------------------------------------------------------------------------------ |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                             |
| Tools           | Default coding tools    | Custom Vilaro tool suite                                                                   |
