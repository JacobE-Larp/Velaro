...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/vilaro`)
...
If the page fails to load, start the Gateway first: `vilaro gateway`.
...
you revoke it with `vilaro devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.vilaro/vilaro.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop vilaro`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Vilaro
...
Vilaro **blocks** Control UI connections without device identity.
