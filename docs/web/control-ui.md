...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/velaro`)
...
If the page fails to load, start the Gateway first: `velaro gateway`.
...
you revoke it with `velaro devices revoke --device <id> --role <role>`. See
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop velaro`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Velaro
...
Velaro **blocks** Control UI connections without device identity.
