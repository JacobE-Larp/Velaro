...
Velaro can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `velaro configure --section web` enables the bundled Firecrawl plugin automatically.
...
Velaro always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
