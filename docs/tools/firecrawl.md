...
Vilaro can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `vilaro configure --section web` enables the bundled Firecrawl plugin automatically.
...
Vilaro always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
