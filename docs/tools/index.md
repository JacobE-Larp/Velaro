---
summary: "Agent tool surface for Vilaro (browser, canvas, nodes, message, cron) replacing legacy `vilaro-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `vilaro-*` skills
title: "Tools"
---

# Tools (Vilaro)

Vilaro exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `vilaro-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `vilaro.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Vilaro logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:vilaro`: all built-in Vilaro tools (excludes provider plugins)
...
- [Vilaro](/tools/vilaro): typed workflow runtime with resumable approvals (requires the Vilaro CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`vilaro node run`).
...
Vilaro tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `vilaro configure --section web`).
...
Control the dedicated Vilaro-managed browser.
...
- Omit `profile` for the safe default: isolated Vilaro-managed browser (`vilaro`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `vilaro`).
...
- Quick smoke: `vilaro nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `vilaro gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Vilaro uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.vilaro/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Vilaro clamps `tools.sessions.visibility` to `tree`.
