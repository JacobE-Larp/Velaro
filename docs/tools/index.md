---
summary: "Agent tool surface for Velaro (browser, canvas, nodes, message, cron) replacing legacy `velaro-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `velaro-*` skills
title: "Tools"
---

# Tools (Velaro)

Velaro exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `velaro-*` skills: the tools are typed, no shelling,
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Velaro logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:velaro`: all built-in Velaro tools (excludes provider plugins)
...
- [Velaro](/tools/velaro): typed workflow runtime with resumable approvals (requires the Velaro CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`velaro node run`).
...
Velaro tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `velaro configure --section web`).
...
Control the dedicated Velaro-managed browser.
...
- Omit `profile` for the safe default: isolated Velaro-managed browser (`velaro`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `velaro`).
...
- Quick smoke: `velaro nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `velaro gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Velaro uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Velaro clamps `tools.sessions.visibility` to `tree`.
