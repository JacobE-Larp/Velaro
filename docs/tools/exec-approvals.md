...
- **gateway host** → `vilaro` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Vilaro also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.vilaro/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.vilaro/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `vilaro approvals allowlist ...`).
- `vilaro security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `vilaro doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.vilaro/exec-approvals.json` directly.

CLI: `vilaro approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Vilaro does not treat
...
topics, Vilaro preserves the topic for the approval prompt and the post-approval follow-up.
