...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@vilaro/msteams` if you use Teams.
...
- Set `VILARO_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `VILARO_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `VILARO_PLUGIN_DISCOVERY_CACHE_MS` and
  `VILARO_PLUGIN_MANIFEST_CACHE_MS`.
...
- `configPatch`: optional `vilaro.json` changes
