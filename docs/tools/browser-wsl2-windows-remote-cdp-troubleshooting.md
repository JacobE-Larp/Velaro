...
read_when:
  - Running Vilaro Gateway in WSL2 while Chrome lives on Windows
...
- Vilaro Gateway runs inside WSL2
...
- Vilaro and Chrome are on the same machine
...
- Vilaro points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Vilaro is not the problem yet.
...
Fix that before touching Vilaro config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Vilaro at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Vilaro to succeed
...
```bash
vilaro browser open https://example.com --browser-profile remote
vilaro browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `vilaro browser tabs` returns the target
...
3. Vilaro config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Vilaro config or Control UI auth
