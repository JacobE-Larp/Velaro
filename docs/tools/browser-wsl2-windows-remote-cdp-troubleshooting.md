...
read_when:
  - Running Velaro Gateway in WSL2 while Chrome lives on Windows
...
- Velaro Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/vilaro/vilaro/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Velaro and Chrome are on the same machine
...
- Velaro points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Velaro is not the problem yet.
...
Fix that before touching Velaro config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Velaro at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Velaro to succeed
...
```bash
velaro browser open https://example.com --browser-profile remote
velaro browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `velaro browser tabs` returns the target
...
3. Velaro config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Velaro config or Control UI auth
