...
`<workspace>/skills` (highest) → `~/.vilaro/skills` → bundled skills (lowest)

Additionally, you can configure extra skill folders (lowest precedence) via
`skills.load.extraDirs` in `~/.vilaro/vilaro.json`.
...
- **Shared skills** live in `~/.vilaro/skills` (managed/local) and are visible
...
- `requires.config` — list of `vilaro.json` paths that must be truthy.
