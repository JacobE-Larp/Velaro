...
Vilaro integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Vilaro owns lifecycle when `channels.signal.autoStart=true`.
...
- Vilaro spawns `imsg rpc` as a child process (legacy iMessage integration).
