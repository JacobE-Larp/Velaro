...
Velaro integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Velaro owns lifecycle when `channels.signal.autoStart=true`.
...
- Velaro spawns `imsg rpc` as a child process (legacy iMessage integration).
