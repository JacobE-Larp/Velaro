...
This is the full reference for the `velaro onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.vilaro/credentials/oauth.json`; auth profiles live in `~/.vilaro/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.vilaro/credentials/oauth.json` (or `$VILARO_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.vilaro/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `velaro pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `velaro configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `velaro health`.
    - Tip: `velaro status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
velaro onboard --non-interactive \
  ...
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env VILARO_GATEWAY_TOKEN
```
...
- Stores it under `~/.vilaro/tools/signal-cli/<version>/`.
...
Typical fields in `~/.vilaro/vilaro.json`:
