...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.vilaro/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$VILARO_STATE_DIR/logs/cache-trace.jsonl`
...
- `VILARO_CACHE_TRACE=1` enables cache tracing.
- `VILARO_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `VILARO_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `VILARO_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `VILARO_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
