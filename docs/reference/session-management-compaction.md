...
This document explains how Vilaro manages sessions end-to-end:
...
Vilaro is designed around a single **Gateway process** that owns session state.
...
Vilaro persists sessions in two layers:
...
Vilaro resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Vilaro reports potential evictions but does not mutate the store/files.
...
Vilaro intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Vilaro consumes the events, but Pi decides when to compact).
...
Vilaro also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Vilaro bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Vilaro leaves it alone.
...
Vilaro supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Vilaro strips/suppresses this in the delivery layer.

As of `2026.1.10`, Vilaro also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Vilaro uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Vilaro’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `vilaro status`.
