...
This document describes how the Vilaro ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`vilaro acp` exposes an ACP agent over stdio and forwards prompts to a running
Vilaro Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Vilaro Gateway
...
- ACP client spawns `vilaro acp` and speaks ACP messages over stdio.
...
`vilaro acp` resolves the Gateway URL and auth from CLI flags or config:
