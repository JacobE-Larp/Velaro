...
- regenerates apps/ios/Vilaro.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${VILARO_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid VILARO_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid VILARO_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid VILARO_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid VILARO_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing VILARO_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(VILARO_URL_SLASH)$(VILARO_URL_SLASH)#g'
)"
...
VILARO_CODE_SIGN_STYLE = Automatic
VILARO_DEVELOPMENT_TEAM = ${TEAM_ID}
VILARO_IOS_SELECTED_TEAM = ${TEAM_ID}
VILARO_APP_BUNDLE_ID = ai.vilaro.client
VILARO_SHARE_BUNDLE_ID = ai.vilaro.client.share
VILARO_ACTIVITY_WIDGET_BUNDLE_ID = ai.vilaro.client.activitywidget
VILARO_WATCH_APP_BUNDLE_ID = ai.vilaro.client.watchkitapp
VILARO_WATCH_EXTENSION_BUNDLE_ID = ai.vilaro.client.watchkitapp.extension
VILARO_APP_PROFILE =
VILARO_SHARE_PROFILE =
VILARO_PUSH_TRANSPORT = relay
VILARO_PUSH_DISTRIBUTION = official
VILARO_URL_SLASH = /
VILARO_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
VILARO_PUSH_APNS_ENVIRONMENT = production
EOF
