...
const extensionRestartMetadataFiles = new Set(["vilaro.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.VILARO_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.VILARO_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[vilaro] ${message}\n`);
};

const runVilaro = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["vilaro.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
