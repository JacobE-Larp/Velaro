...
const EXPECTED_REPOSITORY_URL = "https://github.com/vilaro/vilaro";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "vilaro") {
    errors.push(`package.json name must be "vilaro"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.vilaro !== "vilaro.mjs") {
    errors.push(`package.json bin.vilaro must be "vilaro.mjs"; found "${pkg.bin?.vilaro ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`vilaro-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `vilaro-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
