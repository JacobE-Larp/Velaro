...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-vilaro-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://vilaro.ai/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/vilaro-parallels-windows.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/vilaro-parallels-build.lock"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-vilaro-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://vilaro.ai/install.ps1
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: vilaro@2026.3.13-beta.1
...
guest_run_vilaro() {
  ...
  stdout_name="vilaro-stdout-$RANDOM-$RANDOM.log"
  stderr_name="vilaro-stderr-$RANDOM-$RANDOM.log"
...
try {
  ...
  \$proc = Start-Process -FilePath (Join-Path \$env:APPDATA 'npm\vilaro.cmd') -ArgumentList $args_literal -NoNewWindow -PassThru -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr
  \$proc.WaitForExit()
  if (Test-Path \$stdout) {
    Get-Content \$stdout
  }
...
resolve_latest_version() {
  ...
  npm view vilaro version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "vilaro-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\Vilaro\\deps\" mkdir \"%LOCALAPPDATA%\\Vilaro\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/vilaro-main-$short_head.tgz"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/vilaro-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\vilaro.cmd') --version
EOF
)"
}
...
install_main_tgz() {
  ...
  guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Vilaro\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\vilaro.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_vilaro "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="vilaro-onboard-$RANDOM-$RANDOM.ps1"
  log_name="vilaro-onboard-$RANDOM-$RANDOM.log"
  done_name="vilaro-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$vilaro = Join-Path \$env:APPDATA 'npm\vilaro.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$vilaro, \$log)
...
verify_gateway() {
  guest_run_vilaro "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_vilaro "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_vilaro "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_vilaro "" "" gateway status --deep
}

verify_turn() {
  guest_run_vilaro "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "vilaro-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "vilaro-main-upgrade.tgz" || return $?
