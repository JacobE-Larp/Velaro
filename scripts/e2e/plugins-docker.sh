...
if [ -f dist/index.mjs ]; then
  VILARO_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  VILARO_ENTRY="dist/index.js"
else
...
home_dir=$(mktemp -d "/tmp/vilaro-plugins-e2e.XXXXXX")
...
JS
  cat > "$dir/vilaro.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/vilaro.plugin.json" "$id"
...
mkdir -p "$HOME/.vilaro/extensions/demo-plugin"

cat > "$HOME/.vilaro/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.vilaro/extensions/demo-plugin/vilaro.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$VILARO_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/vilaro-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@vilaro/demo-plugin-tgz",
  "version": "0.0.1",
  "vilaro": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/vilaro.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$VILARO_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$VILARO_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/vilaro-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@vilaro/demo-plugin-dir",
  "version": "0.0.1",
  "vilaro": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/vilaro.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$VILARO_ENTRY" plugins install "$dir_plugin"
node "$VILARO_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/vilaro-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@vilaro/demo-plugin-file",
  "version": "0.0.1",
  "vilaro": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/vilaro.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$VILARO_ENTRY" plugins install "file:$file_pack_dir/package"
node "$VILARO_ENTRY" plugins list --json > /tmp/plugins4.json
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "vilaro/fixture-marketplace"
    }
  }
}
JSON

node "$VILARO_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$VILARO_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$VILARO_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$VILARO_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
const configPath = path.join(process.env.HOME, ".vilaro", "vilaro.json");
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$VILARO_ENTRY" plugins update marketplace-shortcut --dry-run
node "$VILARO_ENTRY" plugins update marketplace-shortcut
node "$VILARO_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
