...
IMAGE_NAME="vilaro-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="vilaro-net-e2e-$$"
GW_NAME="vilaro-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "VILARO_GATEWAY_TOKEN=$TOKEN" \
  -e "VILARO_SKIP_CHANNELS=1" \
  -e "VILARO_SKIP_GMAIL_WATCHER=1" \
  -e "VILARO_SKIP_CRON=1" \
  -e "VILARO_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
