...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "vilaro.plugin.json");
    ...
    if (packageJson.vilaro && "extensions" in packageJson.vilaro) {
      packageJson.vilaro = {
        ...packageJson.vilaro,
        extensions: rewritePackageExtensions(packageJson.vilaro.extensions),
        ...(typeof packageJson.vilaro.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.vilaro.setupEntry) }
          : {}),
      };
    }
