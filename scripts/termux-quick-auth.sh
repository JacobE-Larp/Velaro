...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/vilaro/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Vilaro Re-Auth" -c "After getting key, run: ssh $SERVER '~/vilaro/scripts/mobile-reauth.sh'" --id vilaro-auth
        ;;
    *)
