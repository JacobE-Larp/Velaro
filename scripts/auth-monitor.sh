...
#
# Suggested cron: */30 * * * * /home/admin/vilaro/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Vilaro notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., vilaro-alerts)
...
STATE_FILE="$HOME/.vilaro/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Vilaro if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use vilaro
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Vilaro to $NOTIFY_PHONE..."
            vilaro send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Vilaro is down. Run: ssh l36 '~/vilaro/scripts/mobile-reauth.sh'" "urgent"
    exit 1
