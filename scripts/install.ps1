# Vilaro Installer for Windows (PowerShell)
# Usage: iwr -useb https://vilaro.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://vilaro.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\vilaro",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  Vilaro Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Vilaro.$NC" -Level info
    Write-Host ""
}
...
function Install-VilaroGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Vilaro from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/vilaro/vilaro.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\vilaro\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\vilaro.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Vilaro from git to $GitDir" -Level info
        } else {
            Install-VilaroGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Vilaro via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-VilaroNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'vilaro onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "Vilaro installed successfully!" -Level success
}
