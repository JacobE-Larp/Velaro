...
const tmpHome = mkdtempSync(path.join(os.tmpdir(), "vilaro-startup-memory-"));
...
const MAX_RSS_MARKER = "__VILARO_MAX_RSS_KB__=";
...
const cases = [
  {
    id: "help",
    label: "--help",
    args: ["vilaro.mjs", "--help"],
    limitMb: Number(process.env.VILARO_STARTUP_MEMORY_HELP_MB ?? DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["vilaro.mjs", "status", "--json"],
    limitMb: Number(
      process.env.VILARO_STARTUP_MEMORY_STATUS_JSON_MB ?? DEFAULT_LIMITS_MB.statusJson,
    ),
  },
  {
    id: "gatewayStatus",
    label: "gateway status",
    args: ["vilaro.mjs", "gateway", "status"],
    limitMb: Number(
      process.env.VILARO_STARTUP_MEMORY_GATEWAY_STATUS_MB ?? DEFAULT_LIMITS_MB.gatewayStatus,
    ),
  },
];
