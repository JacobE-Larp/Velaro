#!/bin/bash
# Setup Vilaro Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'vilaro-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Vilaro message: Send warning via Vilaro itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/vilaro-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/vilaro-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/vilaro-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now vilaro-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/VilaroAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/VilaroAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Vilaro*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (VilaroAuth) shows status and opens auth URL if needed."
echo "The full widget (VilaroAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status vilaro-auth-monitor.timer"
echo ""
echo "Quick commands:"
