 ...
 * `vilaro update`. Use it to verify the completion installation flow
...
\`vilaro update\`, \`vilaro doctor\`, and \`vilaro onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Vilaro Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`vilaro completion --install\` later to enable.`));
    return;
  }
