 ...
 * `velaro update`. Use it to verify the completion installation flow
...
\`velaro update\`, \`velaro doctor\`, and \`velaro onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Velaro Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`velaro completion --install\` later to enable.`));
    return;
  }
