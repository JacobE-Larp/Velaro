#!/usr/bin/env bash
# Reset Vilaro like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${VILARO_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Vilaro.app/Contents/MacOS/Vilaro"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Vilaro"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Vilaro"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Vilaro"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.vilaro.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/vilaro-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${VILARO_RESTART_LOG:-/tmp/vilaro-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${VILARO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.vilaro/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  VILARO_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node vilaro.mjs daemon install --force --runtime node"
      log "  node vilaro.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.vilaro/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.vilaro.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Vilaro instances"
kill_all_vilaro
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Vilaro"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Vilaro.app" ]]; then
    APP_BUNDLE="/Applications/Vilaro.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Vilaro.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Vilaro.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Vilaro.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set VILARO_APP_BUNDLE to your installed Vilaro.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node vilaro.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node vilaro.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".vilaro", "vilaro.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Vilaro is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.vilaro.gateway.plist' | head -n 40 || true"
fi
