...
BASE_IMAGE="${BASE_IMAGE:-vilaro-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-vilaro-sandbox-common:bookworm-slim}"
...
VILARO_DOCKER_BUILD_USE_BUILDX="${VILARO_DOCKER_BUILD_USE_BUILDX:-0}"
VILARO_DOCKER_BUILD_CACHE_FROM="${VILARO_DOCKER_BUILD_CACHE_FROM:-}"
VILARO_DOCKER_BUILD_CACHE_TO="${VILARO_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${VILARO_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${VILARO_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${VILARO_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${VILARO_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${VILARO_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=vilaro.sandbox=1)
NOTE
