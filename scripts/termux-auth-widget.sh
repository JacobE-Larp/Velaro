...
SERVER="${VILARO_SERVER:-${VILARO_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Vilaro auth..."

STATUS=$(ssh "$SERVER" '$HOME/vilaro/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/vilaro/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|VILARO_EXPIRING|VILARO_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/vilaro/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|VILARO_EXPIRED|VILARO_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Vilaro won't work until re-auth"
                ;;
        esac
        ;;
