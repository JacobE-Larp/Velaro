...
# Vilaro Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://vilaro.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Vilaro."
...
GUM_VERSION="${VILARO_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "Vilaro Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://vilaro.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$VILARO_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.vilaro.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_vilaro_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.vilaro-* "$npm_root"/vilaro 2>/dev/null || true
}

extract_vilaro_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_vilaro_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/vilaro" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/vilaro"|"/usr/local/bin/vilaro")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/vilaro/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale vilaro symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing vilaro binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Vilaro package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_vilaro_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Vilaro npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*vilaro" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_vilaro_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_vilaro_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Vilaro npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an vilaro binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("I'll butter your workflow like a vilaro roll: messy, delicious, effective.")
...
map_legacy_env "VILARO_TAGLINE_INDEX" "VILARO_TAGLINE_INDEX"
map_legacy_env "VILARO_NO_ONBOARD" "VILARO_NO_ONBOARD"
map_legacy_env "VILARO_NO_PROMPT" "VILARO_NO_PROMPT"
map_legacy_env "VILARO_DRY_RUN" "VILARO_DRY_RUN"
map_legacy_env "VILARO_INSTALL_METHOD" "VILARO_INSTALL_METHOD"
map_legacy_env "VILARO_VERSION" "VILARO_VERSION"
map_legacy_env "VILARO_BETA" "VILARO_BETA"
map_legacy_env "VILARO_GIT_DIR" "VILARO_GIT_DIR"
map_legacy_env "VILARO_GIT_UPDATE" "VILARO_GIT_UPDATE"
map_legacy_env "VILARO_NPM_LOGLEVEL" "VILARO_NPM_LOGLEVEL"
map_legacy_env "VILARO_VERBOSE" "VILARO_VERBOSE"
map_legacy_env "VILARO_PROFILE" "VILARO_PROFILE"
map_legacy_env "VILARO_INSTALL_SH_NO_RUN" "VILARO_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${VILARO_TAGLINE_INDEX:-}" ]]; then
        if [[ "${VILARO_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((VILARO_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${VILARO_NO_ONBOARD:-0}
NO_PROMPT=${VILARO_NO_PROMPT:-0}
DRY_RUN=${VILARO_DRY_RUN:-0}
INSTALL_METHOD=${VILARO_INSTALL_METHOD:-}
VILARO_VERSION=${VILARO_VERSION:-latest}
USE_BETA=${VILARO_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/vilaro"
GIT_DIR=${VILARO_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${VILARO_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${VILARO_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${VILARO_VERBOSE:-0}"
VERIFY_INSTALL="${VILARO_VERIFY_INSTALL:-0}"
VILARO_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Vilaro installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://vilaro.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/vilaro)
...
Environment variables:
  VILARO_INSTALL_METHOD=git|npm
  VILARO_VERSION=latest|next|main|<semver>|<spec>
  VILARO_BETA=0|1
  VILARO_GIT_DIR=...
  VILARO_GIT_UPDATE=0|1
  VILARO_NO_PROMPT=1
  VILARO_VERIFY_INSTALL=1
  VILARO_DRY_RUN=1
  VILARO_NO_ONBOARD=1
  VILARO_VERBOSE=1
  VILARO_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://vilaro.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://vilaro.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://vilaro.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://vilaro.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://vilaro.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Vilaro checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Vilaro source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_vilaro_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"vilaro"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://vilaro.ai/install.sh | bash"
}
...
ensure_vilaro_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/vilaro" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/vilaro" ]]; then
        ln -sf "$npm_root/vilaro/dist/entry.js" "${npm_bin}/vilaro"
        ui_info "Created vilaro bin link at ${npm_bin}/vilaro"
    fi
    return 0
}

# Check for existing Vilaro installation
check_existing_vilaro() {
    if [[ -n "$(type -P vilaro 2>/dev/null || true)" ]]; then
        ui_info "Existing Vilaro installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make vilaro show as \"command not found\" in new terminals."
...
warn_vilaro_not_found() {
    ui_warn "Installed, but vilaro is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t vilaro 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named vilaro; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_vilaro_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P vilaro 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/vilaro" ]]; then
        echo "${npm_bin}/vilaro"
        return 0
    fi
...
install_vilaro_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/vilaro/vilaro.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Vilaro from git checkout: ${repo_dir}"
    else
        ui_info "Installing Vilaro from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Vilaro" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Vilaro" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/vilaro" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/vilaro"
    ui_success "Vilaro wrapper installed to \$HOME/.local/bin/vilaro"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view vilaro dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:vilaro/vilaro#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_vilaro() {
    local package_name="vilaro"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            VILARO_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="vilaro"
        else
            VILARO_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${VILARO_VERSION}" ]]; then
        VILARO_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${VILARO_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${VILARO_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Vilaro v${resolved_version}"
    else
        ui_info "Installing Vilaro (${VILARO_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${VILARO_VERSION}")"

    if ! install_vilaro_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_vilaro_paths
        install_vilaro_npm "${install_spec}"
    fi

    if [[ "${VILARO_VERSION}" == "latest" && "${package_name}" == "vilaro" ]]; then
        if ! resolve_vilaro_bin &> /dev/null; then
            ui_warn "npm install vilaro@latest failed; retrying vilaro@next"
            cleanup_npm_vilaro_paths
            install_vilaro_npm "vilaro@next"
        fi
    fi

    ensure_vilaro_bin_link || true

    ui_success "Vilaro installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${VILARO_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_vilaro_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (vilaro not on PATH yet)"
        warn_vilaro_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${VILARO_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.vilaro/workspace-${profile}"
    else
        echo "${HOME}/.vilaro/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${VILARO_CONFIG_PATH:-$HOME/.vilaro/vilaro.json}"
    if [[ -f "${config_path}" || -f "$HOME/.vilaro/vilaro.json" || -f "$HOME/.vilaro/vilaro.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run vilaro onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but vilaro not on PATH; skipping onboarding"
        warn_vilaro_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run vilaro onboard to retry"
        return
    }
}
...
resolve_vilaro_version() {
    local version=""
    local raw_version_output=""
    local claw="${VILARO_BIN:-}"
    if [[ -z "$claw" ]] && command -v vilaro &> /dev/null; then
        claw="$(command -v vilaro)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_vilaro_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/vilaro/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/vilaro/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: vilaro not on PATH yet"
        warn_vilaro_not_found
        return 1
    fi

    run_quiet_step "Checking Vilaro version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: vilaro gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_vilaro_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Vilaro checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set VILARO_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_vilaro_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/vilaro" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/vilaro"
    ...
    VILARO_BIN="$(resolve_vilaro_bin || true)"
    ...
    installed_version=$(resolve_vilaro_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "Vilaro installed successfully (${installed_version})!"
    else
        ui_celebrate "Vilaro installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            "Leveled up! New skills unlocked. You're welcome."
            "Fresh code, same vilaro. Miss me?"
            ...
            "The vilaro has molted. Harder shell, sharper claws."
        ...
        local completion_messages=(
            ...
            "The vilaro has landed. Your terminal will never be the same."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/vilaro"
        ui_kv "Update command" "vilaro update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://vilaro.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running vilaro doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                VILARO_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                VILARO_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                VILARO_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run vilaro doctor and vilaro plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run vilaro onboard later"
        else
            local config_path="${VILARO_CONFIG_PATH:-$HOME/.vilaro/vilaro.json}"
            if [[ -f "${config_path}" || -f "$HOME/.vilaro/vilaro.json" || -f "$HOME/.vilaro/vilaro.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (vilaro not on PATH yet)"
                    warn_vilaro_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run vilaro onboard to finish setup"
            return 0
        fi
    fi

    if command -v vilaro &> /dev/null; then
        local claw="${VILARO_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_vilaro_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (vilaro daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if VILARO_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: vilaro daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${VILARO_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
