...
INSTALL_URL="${VILARO_INSTALL_URL:-https://vilaro.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${VILARO_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${VILARO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="vilaro"
PACKAGE_NAME="${VILARO_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (VILARO_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${VILARO_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${VILARO_INSTALL_LATEST_OUT:-}"
fi
