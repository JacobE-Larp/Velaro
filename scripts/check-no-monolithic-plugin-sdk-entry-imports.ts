...
import { discoverVilaroPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']vilaro\/plugin-sdk["']/;
const LEGACY_ROUTING_IMPORT_PATTERN = /["']vilaro\/plugin-sdk\/routing["']/;
...
function main() {
  ...
  if (monolithicOffenders.length > 0 || legacyRoutingOffenders.length > 0) {
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic vilaro/plugin-sdk.");
    ...
    if (legacyRoutingOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy vilaro/plugin-sdk/routing.",
      );
    ...
    if (monolithicOffenders.length > 0 || legacyRoutingOffenders.length > 0) {
      console.error(
        "Use vilaro/plugin-sdk/<channel> for channel plugins, /core for shared routing and startup surfaces, or /compat for broader internals.",
      );
    }
    process.exit(1);
  }
