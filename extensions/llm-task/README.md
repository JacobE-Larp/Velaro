...
Designed to be called from workflow engines (for example, Vilaro via
`vilaro.invoke --each`) without adding new Vilaro code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Vilaro) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on Vilaro internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Vilaro extension (like `vilaro`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.vilaro/extensions` as a standalone plugin directory.
