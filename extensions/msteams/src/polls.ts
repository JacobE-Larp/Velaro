...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["vilaroPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["vilaro", "pollId"]) ??
    readNestedString(value, ["vilaro", "poll", "id"]) ??
    readNestedString(value, ["data", "vilaroPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "vilaro", "pollId"]);
  if (!pollId) {
    return null;
  }
