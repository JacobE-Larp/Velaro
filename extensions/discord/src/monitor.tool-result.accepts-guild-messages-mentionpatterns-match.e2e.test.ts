  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/vilaro",
      },
    },
    session: { store: "/tmp/vilaro-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bvilaro\\b"] },
        },
      });
      ...
      await handler(createGuildMessageEvent({ messageId: "m2", content: "vilaro: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "Vilaro" },
          },
        },
        eventPatch: {
