...
function resolveVoiceTtsConfig(params: { cfg: VilaroConfig; override?: TtsConfig }): {
  cfg: VilaroConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredVilaroTmpDir(), "discord-voice-"));
