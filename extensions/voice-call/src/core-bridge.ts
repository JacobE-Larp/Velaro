...
function resolveVelaroRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.VILARO_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set VILARO_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveVelaroRoot(), "dist", "extensionAPI.js");
