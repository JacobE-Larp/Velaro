...
metadata: { "vilaro": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Vilaro `sessions_spawn`
- **File I/O** == Vilaro `read`/`write`
- **Remote fetch** == Vilaro `web_fetch` (or `exec` with curl when POST is required)
