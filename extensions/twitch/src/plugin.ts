...
import type { VilaroConfig } from "vilaro/plugin-sdk/twitch";
import { buildChannelConfigSchema } from "vilaro/plugin-sdk/twitch";
...
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: VilaroConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: VilaroConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: VilaroConfig): boolean => {
