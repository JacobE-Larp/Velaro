...
import type { VilaroPluginApi, VilaroPluginService } from "vilaro/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: VilaroPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: VilaroPluginApi["config"]): string[] {
