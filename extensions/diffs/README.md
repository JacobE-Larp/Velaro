...
Read-only diff viewer plugin for **Vilaro** agents.
...
Set plugin-wide defaults in `~/.vilaro/vilaro.json`:
...
Before:
Vilaro supports plugins.

After:
Vilaro supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/vilaro-diffs`).
