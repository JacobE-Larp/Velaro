...
import type { VilaroPluginApi } from "vilaro/plugin-sdk/diffs";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<VilaroPluginApi["registerHttpRoute"]>[0];
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<VilaroPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as VilaroPluginApi);
