...
vi.mock("vilaro/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("vilaro/plugin-sdk/diagnostics-otel")>(
    "vilaro/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { VilaroPluginServiceContext } from "vilaro/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "vilaro/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/vilaro-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): VilaroPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("vilaro.webhook.received")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("vilaro.webhook.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("vilaro.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("vilaro.message.processed")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("vilaro.message.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.histograms.get("vilaro.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("vilaro.session.stuck")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("vilaro.session.stuck_age_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("vilaro.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("vilaro.webhook.processed");
    expect(spanNames).toContain("vilaro.message.processed");
    expect(spanNames).toContain("vilaro.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["vilaro.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("vilaro.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "vilaro.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["vilaro.reason"]).toBe("string");
    expect(String(attrs?.["vilaro.reason"])).not.toContain(
