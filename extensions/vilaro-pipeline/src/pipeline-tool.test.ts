...
import type { VilaroPluginApi, VilaroPluginToolContext } from "vilaro/plugin-sdk/pipeline";
...
function fakeApi(overrides: Partial<VilaroPluginApi> = {}): VilaroPluginApi {
  return {
    id: "vilaro",
    name: "vilaro",
    source: "test",
...
function fakeCtx(overrides: Partial<VilaroPluginToolContext> = {}): VilaroPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("vilaro plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-vilaro-plugin-"));
  });
  ...
  it("runs vilaro and returns parsed envelope in details", async () => {
    spawnState.queue.push({
      stdout: JSON.stringify({
        ok: true,
        status: "ok",
  ...
  it("rejects invalid JSON from vilaro", async () => {
  ...
  it("runs Windows cmd shims through Node without enabling shell", async () => {
    ...
    const shimScriptPath = path.join(tempDir, "shim-dist", "vilaro-cli.cjs");
    const shimPath = path.join(tempDir, "shim-bin", "vilaro.cmd");
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: VilaroPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
    ...
    expect(factoryTool(fakeCtx({ sandboxed: false }))?.name).toBe("vilaro");
  });
});
