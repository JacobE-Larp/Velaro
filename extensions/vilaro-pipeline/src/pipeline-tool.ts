...
import type { VilaroPluginApi } from "vilaro/plugin-sdk/pipeline";
...
async function runVilaroSubprocessOnce(params: {
  execPath: string;
  argv: string[];
  cwd: string;
  timeoutMs: number;
  ...
  const spawnTarget =
    process.platform === "win32"
      ? resolveWindowsVilaroSpawn(execPath, argv, env)
...
function parseEnvelope(stdout: string): VilaroEnvelope {
