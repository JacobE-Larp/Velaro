...
import type { VilaroConfig, PluginRuntime } from "vilaro/plugin-sdk/feishu";
import type { DynamicAgentCreationConfig } from "./types.js";

export type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: VilaroConfig;
  agentId?: string;
};
...
export async function maybeCreateDynamicAgent(params: {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.vilaro/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.vilaro/agents/{agentId}/agent";
