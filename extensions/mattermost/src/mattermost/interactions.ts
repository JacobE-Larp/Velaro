...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__vilaro_channel_id";
...
type InteractionCallbackConfig = Pick<VilaroConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "vilaro-mattermost-interactions").update(botToken).digest("hex");
}
