...
const liveEnabled = HAS_OPENAI_KEY && process.env.VILARO_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "vilaro-memory-test-" });
...
describeLive("memory plugin live tests", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "vilaro-memory-live-" });
