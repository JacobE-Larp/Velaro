...
val androidStoreFile = providers.gradleProperty("VILARO_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("VILARO_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("VILARO_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("VILARO_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set VILARO_ANDROID_STORE_FILE, " +
            "VILARO_ANDROID_STORE_PASSWORD, VILARO_ANDROID_KEY_ALIAS, and " +
            "VILARO_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/VilaroKit/Sources/VilaroKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.vilaro.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "vilaro-$versionName-$buildType.apk"
