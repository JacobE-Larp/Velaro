...
private const val NOTIFICATION_CHANNEL_BASE_ID = "vilaro.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Vilaro Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Vilaro Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Vilaro Active")
      }
