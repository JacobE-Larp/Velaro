...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.vilaro.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Vilaro", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Vilaro gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Vilaro.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "VilaroShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Vilaro is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Vilaro Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("vilaro-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
