...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> VilaroDeviceStatusPayload {
        ...
        return VilaroDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> VilaroDeviceInfoPayload {
        ...
        return VilaroDeviceInfoPayload(
    ...
    private func batteryStatus() -> VilaroBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: VilaroBatteryState = switch device.batteryState {
        ...
        return VilaroBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> VilaroThermalStatusPayload {
        let state: VilaroThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return VilaroThermalStatusPayload(state: state)
    }

    private func storageStatus() -> VilaroStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return VilaroStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
