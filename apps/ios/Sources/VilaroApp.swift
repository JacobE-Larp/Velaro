...
@MainActor
final class VilaroAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.vilaro.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.vilaro.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.vilaro.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "vilaro.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "vilaro.watch.promptId"
    static let sessionKeyKey = "vilaro.watch.sessionKey"
    static let actionPrimaryIDKey = "vilaro.watch.action.primary.id"
    static let actionPrimaryLabelKey = "vilaro.watch.action.primary.label"
    static let actionSecondaryIDKey = "vilaro.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "vilaro.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "vilaro.watch.action.primary"
    static let actionSecondaryIdentifier = "vilaro.watch.action.secondary"
    static let actionIdentifierPrefix = "vilaro.watch.action."
    static let actionIDKeyPrefix = "vilaro.watch.action.id."
    static let actionLabelKeyPrefix = "vilaro.watch.action.label."
    static let categoryPrefix = "vilaro.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: VilaroWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> VilaroWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return VilaroWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Vilaro" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [VilaroWatchAction]) -> [UNNotificationAction] {
...
@main
struct VilaroApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(VilaroAppDelegate.self) private var appDelegate
...
extension VilaroApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Vilaro: installing uncaught exception handler")
