...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: VilaroWatchNotifyParams) -> VilaroWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [VilaroWatchAction]?,
        kind: String?,
        promptId: String?) -> [VilaroWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> VilaroWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return VilaroWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                VilaroWatchAction(id: "approve", label: "Approve"),
                VilaroWatchAction(id: "decline", label: "Decline", style: "destructive"),
                VilaroWatchAction(id: "open_phone", label: "Open iPhone"),
                VilaroWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            VilaroWatchAction(id: "done", label: "Done"),
            VilaroWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            VilaroWatchAction(id: "open_phone", label: "Open iPhone"),
            VilaroWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: VilaroWatchRisk?,
        priority: VilaroNotificationPriority?) -> VilaroWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: VilaroNotificationPriority?,
        risk: VilaroWatchRisk?) -> VilaroNotificationPriority?
    {
