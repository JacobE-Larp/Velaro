...
final class RemindersService: RemindersServicing {
    func list(params: VilaroRemindersListParams) async throws -> VilaroRemindersListPayload {
        ...
        let payload: [VilaroReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return VilaroRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: VilaroRemindersAddParams) async throws -> VilaroRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = VilaroReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return VilaroRemindersAddPayload(reminder: payload)
    }
