...
final class ContactsService: ContactsServicing {
    ...
    func search(params: VilaroContactsSearchParams) async throws -> VilaroContactsSearchPayload {
        ...
        return VilaroContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: VilaroContactsAddParams) async throws -> VilaroContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return VilaroContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return VilaroContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> VilaroContactPayload {
        VilaroContactPayload(
