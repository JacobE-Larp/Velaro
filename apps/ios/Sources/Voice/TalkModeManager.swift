...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__VILARO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<VilaroTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.vilaro", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = VilaroTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> VilaroTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return VilaroTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> VilaroTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> VilaroTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return VilaroTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> VilaroTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = VilaroTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: VilaroTalkPTTStopPayload) {
