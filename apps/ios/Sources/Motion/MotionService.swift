...
final class MotionService: MotionServicing {
    func activities(params: VilaroMotionActivityParams) async throws -> VilaroMotionActivityPayload {
        ...
        let mapped: [VilaroMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return VilaroMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: VilaroPedometerParams) async throws -> VilaroPedometerPayload {
        ...
        let payload: VilaroPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = VilaroPedometerPayload(
