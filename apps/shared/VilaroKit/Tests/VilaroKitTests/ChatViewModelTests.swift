...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> VilaroChatHistoryPayload
{
    VilaroChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> VilaroChatSessionEntry {
    VilaroChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> VilaroChatSessionEntry
{
    VilaroChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> VilaroChatModelChoice {
    VilaroChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [VilaroChatHistoryPayload],
    sessionsResponses: [VilaroChatSessionsListResponse] = [],
    modelResponses: [[VilaroChatModelChoice]] = [],
    ...
    -> (TestChatTransport, VilaroChatViewModel)
{
...
private func sendUserMessage(_ vm: VilaroChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, VilaroChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [VilaroChatHistoryPayload]
    private let sessionsResponses: [VilaroChatSessionsListResponse]
    private let modelResponses: [[VilaroChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<VilaroChatTransportEvent>
    private let continuation: AsyncStream<VilaroChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<VilaroChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<VilaroChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> VilaroChatHistoryPayload {
        ...
        return self.historyResponses.last ?? VilaroChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [VilaroChatAttachmentPayload]) async throws -> VilaroChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return VilaroChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> VilaroChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? VilaroChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [VilaroChatModelChoice] {
    ...
    func emit(_ evt: VilaroChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = VilaroChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = VilaroChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: VilaroChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = VilaroChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: VilaroChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(VilaroChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == VilaroChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = VilaroChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: VilaroChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = VilaroChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = VilaroChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = VilaroChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { VilaroChatViewModel(sessionKey: "main", transport: transport) }
