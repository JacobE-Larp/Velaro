...
@MainActor
public struct VilaroChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: VilaroChatViewModel
    ...
    private var visibleMessages: [VilaroChatMessage] {
        let base: [VilaroChatMessage]
    ...
    private func mergeToolResults(in messages: [VilaroChatMessage]) -> [VilaroChatMessage] {
    ...
    private func isToolResultMessage(_ message: VilaroChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: VilaroChatMessage) -> Bool {
    ...
    private func primaryText(in message: VilaroChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: VilaroChatMessage) -> Bool {
    ...
    private func toolCalls(in message: VilaroChatMessage) -> [VilaroChatMessageContent] {
    ...
    private func inlineToolResults(in message: VilaroChatMessage) -> [VilaroChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: VilaroChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: VilaroChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(VilaroChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
