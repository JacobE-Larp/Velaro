...
struct ChatMessageBubble: View {
    let message: VilaroChatMessage
    let style: VilaroChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? VilaroChatTheme.userText : VilaroChatTheme.assistantText
    ...
    private var inlineAttachments: [VilaroChatMessageContent] {
    ...
    private var toolCalls: [VilaroChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [VilaroChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? VilaroChatTheme.userBubble
        }
        if self.style == .onboarding {
            return VilaroChatTheme.onboardingAssistantBubble
        }
        return VilaroChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return VilaroChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: VilaroChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? VilaroChatTheme.userText : VilaroChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: VilaroChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(VilaroChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [VilaroChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: VilaroChatTheme.assistantText)
            }
        }
    }
}
