...
private let chatUILogger = Logger(subsystem: "ai.vilaro", category: "VilaroChatUI")

@MainActor
@Observable
public final class VilaroChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [VilaroChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [VilaroChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [VilaroPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [VilaroChatPendingToolCall] = []
    public private(set) var sessions: [VilaroChatSessionEntry] = []
    private let transport: any VilaroChatTransport
    private var sessionDefaults: VilaroChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: VilaroChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any VilaroChatTransport,
    ...
    public var sessionChoices: [VilaroChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [VilaroChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: VilaroPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [VilaroChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: VilaroChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: VilaroChatMessage) -> VilaroChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> VilaroChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return VilaroChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: VilaroChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [VilaroChatMessage],
        incoming: [VilaroChatMessage]) -> [VilaroChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [VilaroChatMessage]) -> [VilaroChatMessage] {
        var result: [VilaroChatMessage] = []
    ...
    private static func dedupeKey(for message: VilaroChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [VilaroChatMessageContent] = [
            VilaroChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> VilaroChatAttachmentPayload in
            VilaroChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> VilaroChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = VilaroChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: VilaroChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: VilaroChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: VilaroAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = VilaroChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> VilaroPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
