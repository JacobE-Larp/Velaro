...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: VilaroProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: VilaroProtocol.AnyCodable] = [
            "presence": .init([VilaroProtocol.AnyCodable(entry)]),
        ]
        let payload = VilaroProtocol.AnyCodable(payloadMap)
