...
struct VilaroMacCLI {
    static func main() async {
        ...
        default:
            fputs("vilaro-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      vilaro-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      vilaro-mac discover [--timeout <ms>] [--json] [--include-local]
      vilaro-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      vilaro-mac connect
      vilaro-mac connect --url ws://127.0.0.1:18789 --json
      vilaro-mac discover --timeout 3000 --json
      vilaro-mac wizard --mode local
    """)
}
