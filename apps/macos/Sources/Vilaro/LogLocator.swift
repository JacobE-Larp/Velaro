...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["VILARO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/vilaro")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("vilaro-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("vilaro-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/vilaro/ (rolling or stdout), or nil if none exist.
