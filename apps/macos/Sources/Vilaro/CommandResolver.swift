...
enum CommandResolver {
    private static let projectRootDefaultsKey = "vilaro.gatewayProjectRootPath"
    private static let helperName = "vilaro"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let vilaroEntry = root.appendingPathComponent("vilaro.mjs").path
        if FileManager().isReadableFile(atPath: vilaroEntry) { return vilaroEntry }
        let binEntry = root.appendingPathComponent("bin/vilaro.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/vilaro")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let vilaroPaths = self.vilaroManagedPaths(home: home)
        if !vilaroPaths.isEmpty {
            extras.insert(contentsOf: vilaroPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + vilaroPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func vilaroManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".vilaro"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func vilaroExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectVilaroExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("vilaro.mjs").path,
            root.appendingPathComponent("bin/vilaro.js").path,
        ]
    ...
    static func hasAnyVilaroInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.vilaroExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let vilaroPath = self.projectVilaroExecutable(projectRoot: root) {
            return [vilaroPath, subcommand] + extraArgs
        }
        if let vilaroPath = self.vilaroExecutable(searchPaths: searchPaths) {
            return [vilaroPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "vilaro", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            vilaro entrypoint missing (looked for dist/index.js or vilaro.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real vilaro CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/vilaro"
        ...
        \(cliSection)
        \(projectSection)
        if command -v vilaro >/dev/null 2>&1; then
          CLI="$(command -v vilaro)"
          vilaro \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/vilaro.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/vilaro.mjs"
            node "$PRJ/vilaro.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/vilaro.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/vilaro.js"
            node "$PRJ/bin/vilaro.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent vilaro"
          pnpm --silent vilaro \(quotedArgs);
        else
          echo "vilaro CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? VilaroConfigFile.loadDict()
