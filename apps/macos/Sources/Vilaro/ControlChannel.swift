...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: VilaroProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.vilaro", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: VilaroKit.AnyCodable]()) {
                $0[$1.key] = VilaroKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: VilaroProtocol.AnyCodable?) -> [String: VilaroProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: VilaroProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: VilaroKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: VilaroProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("vilaro.control.heartbeat")
    static let controlAgentEvent = Notification.Name("vilaro.control.agent")
}
