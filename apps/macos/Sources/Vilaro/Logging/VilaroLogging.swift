...
enum VilaroLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = VilaroOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = VilaroFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        VilaroLogging.bootstrapIfNeeded()
        let label = VilaroLogging.makeLabel(subsystem: subsystem, category: category)
...
struct VilaroOSLogHandler: AppLogLevelBackedHandler {
...
struct VilaroFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = VilaroLogging.parseLabel(self.label)
