...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.vilaro.mac`) even if the shipped
    ...
    {
        if environment["VILARO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "vilaro.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "vilaro.nixMode") { return true }
