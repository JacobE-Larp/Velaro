...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.vilaro", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = VilaroConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: VilaroNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [VilaroCapability.canvas.rawValue, VilaroCapability.screen.rawValue]
        if VilaroConfigFile.browserControlEnabled() {
            caps.append(VilaroCapability.browser.rawValue)
        }
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(VilaroCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if VilaroLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(VilaroCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            VilaroSystemCommand.which.rawValue,
            VilaroSystemCommand.run.rawValue,
            VilaroSystemCommand.execApprovalsGet.rawValue,
            VilaroSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(VilaroCapability.browser.rawValue) {
            commands.append(VilaroBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(VilaroCapability.camera.rawValue) {
        ...
        if capsSet.contains(VilaroCapability.location.rawValue) {
