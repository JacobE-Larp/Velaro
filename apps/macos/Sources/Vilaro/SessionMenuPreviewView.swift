...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.vilaro", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> VilaroSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: VilaroSessionPreviewEntry,
    ...
    private static func cache(payload: VilaroSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: VilaroChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [VilaroKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [VilaroKit.AnyCodable]) -> [VilaroChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(VilaroChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: VilaroChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: VilaroChatMessage) -> Bool {
    ...
    private static func toolNames(for message: VilaroChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: VilaroChatMessage) -> String? {
