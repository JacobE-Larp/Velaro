...
enum DebugActions {
    private static let verboseDefaultsKey = "vilaro.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "vilaro.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = VilaroPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Vilaro", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = VilaroPaths.configURL
        guard
