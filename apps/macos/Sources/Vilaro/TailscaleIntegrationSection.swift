...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.vilaro/vilaro.json. Prefer VILARO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.vilaro/vilaro.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.vilaro/vilaro.json. Restart the gateway to apply."
        }
