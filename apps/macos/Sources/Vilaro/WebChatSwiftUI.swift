...
private let webChatSwiftLogger = Logger(subsystem: "ai.vilaro", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "vilaro.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: VilaroChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> VilaroChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(VilaroChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            VilaroChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return VilaroChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [VilaroChatAttachmentPayload]) async throws -> VilaroChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> VilaroChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                VilaroGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: VilaroProtocol.ModelChoice) -> VilaroChatModelChoice {
        VilaroChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<VilaroChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any VilaroChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = VilaroChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: VilaroChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<VilaroChatView>) -> NSViewController
    {
