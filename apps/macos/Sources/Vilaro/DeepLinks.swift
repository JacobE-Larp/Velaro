...
private let deepLinkLogger = Logger(subsystem: "ai.vilaro", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Vilaro is paused", message: "Unpause Vilaro to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Vilaro agent?", message: body) else { return }
        }
