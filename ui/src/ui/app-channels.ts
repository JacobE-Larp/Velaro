...
export async function handleWhatsAppStart(host: VilaroApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: VilaroApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: VilaroApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: VilaroApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: VilaroApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: VilaroApp): string {
...
function resolveGatewayHttpAuthHeader(host: VilaroApp): string | null {
...
function buildGatewayHttpHeaders(host: VilaroApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: VilaroApp) {
...
export function handleNostrProfileToggleAdvanced(host: VilaroApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: VilaroApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: VilaroApp) {
  const state = host.nostrProfileFormState;
