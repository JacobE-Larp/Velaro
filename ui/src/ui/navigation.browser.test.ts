...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/vilaro/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/vilaro");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/vilaro/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__VILARO_CONTROL_UI_BASE_PATH__ = "/vilaro";
    const app = mountApp("/vilaro/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/vilaro");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/vilaro/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("vilaro.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      ...
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/vilaro" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;
    ...
    expect(JSON.parse(localStorage.getItem("vilaro.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/vilaro",
    });
    expect(JSON.parse(localStorage.getItem("vilaro.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/vilaro";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/vilaro");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp("/ui/overview?gatewayUrl=wss://other-gateway.example/vilaro#token=abc123");
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/vilaro");
    expect(app.settings.token).toBe("");
