const SETTINGS_KEY_PREFIX = "vilaro.control.settings.v1:";
const LEGACY_TOKEN_SESSION_KEY = "vilaro.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "vilaro.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    typeof window.__VILARO_CONTROL_UI_BASE_PATH__ === "string" &&
    window.__VILARO_CONTROL_UI_BASE_PATH__.trim();
...
export function loadSettings(): UiSettings {
  ...
  try {
    ...
    const raw =
      ...
      storage?.getItem("vilaro.control.settings.v1");
    if (!raw) {
      return defaults;
    }
