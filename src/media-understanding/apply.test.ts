...
const TEMP_MEDIA_PREFIX = "vilaro-media-";
...
function createGroqAudioConfig(): VilaroConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): VilaroConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): VilaroConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: VilaroConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredVilaroTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        VILARO_AGENT_DIR: isolatedAgentDir,
