...
const GOOGLE_LIVE_SINGLE_KEY = "VILARO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "VILARO_LIVE_ANTHROPIC_KEY",
    listVar: "VILARO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "VILARO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `VILARO_LIVE_${base}_KEY`;
