...
import { markVilaroExecEnv } from "../../infra/vilaro-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "vilaro.sandbox=1");
  args.push("--label", `vilaro.sessionKey=${params.scopeKey}`);
  args.push("--label", `vilaro.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `vilaro.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markVilaroExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "vilaro.configHash");
}
