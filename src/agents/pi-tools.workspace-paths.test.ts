...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("vilaro-ws-", async (workspaceDir) => {
      await withTempDir("vilaro-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createVilaroCodingTools({ workspaceDir });
          ...
          expect(await fs.readFile(path.join(workspaceDir, editFile), "utf8")).toBe("hello vilaro");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("vilaro-ws-", async (workspaceDir) => {
      await withTempDir("vilaro-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("vilaro-ws-", async (workspaceDir) => {
      const cfg: VilaroConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createVilaroCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-vilaro.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("vilaro-sandbox-", async (sandboxDir) => {
      await withTempDir("vilaro-workspace-", async (workspaceDir) => {
        ...
        const tools = createVilaroCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
