...
async function withOutsideHardlinkInVilaroTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside vilaro tmp root",
      media: path.join(os.tmpdir(), "outside-vilaro", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Vilaro tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Vilaro tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInVilaroTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Vilaro tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInVilaroTmp(
