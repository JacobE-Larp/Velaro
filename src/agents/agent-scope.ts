...
type AgentEntry = NonNullable<NonNullable<VilaroConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: VilaroConfig): AgentEntry[] {
...
export function listAgentIds(cfg: VilaroConfig): string[] {
...
export function resolveDefaultAgentId(cfg: VilaroConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: VilaroConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentSkillsFilter(cfg: VilaroConfig, agentId: string): string[] | undefined {
...
export function resolveAgentModelPrimary(cfg: VilaroConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: VilaroConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentIdsByWorkspacePath(cfg: VilaroConfig, workspacePath: string): string[] {
...
export function resolveAgentDir(cfg: VilaroConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
