...
describe("resolveApiKeyForProfile openai-codex refresh fallback", () => {
  const envSnapshot = captureEnv(["VILARO_STATE_DIR", "VILARO_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tempRoot = "";
  let agentDir = "";
  ...
  beforeEach(async () => {
    ...
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-codex-refresh-fallback-"));
    agentDir = path.join(tempRoot, "agents", "main", "agent");
    await fs.mkdir(agentDir, { recursive: true });
    process.env.VILARO_STATE_DIR = tempRoot;
    process.env.VILARO_AGENT_DIR = agentDir;
