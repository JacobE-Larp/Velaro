...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/vilaro/vilaro/issues/31298
    previousFastTestEnv = process.env.VILARO_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.VILARO_TEST_FAST;
      return;
    }
    process.env.VILARO_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // VILARO_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Vilaro runtime context (internal):");
