...
} from "./vilaro-tools.subagents.sessions-spawn.test-harness.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.VILARO_TEST_FAST;
...
describe("vilaro-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    ...
    process.env.VILARO_TEST_FAST = fastModeEnv.previous;
  });
