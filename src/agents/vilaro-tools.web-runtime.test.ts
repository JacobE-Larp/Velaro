...
function findTool(name: string, config: VilaroConfig) {
  const allTools = createVilaroTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: VilaroConfig; env?: NodeJS.ProcessEnv }) {
...
describe("vilaro tools runtime web metadata wiring", () => {
