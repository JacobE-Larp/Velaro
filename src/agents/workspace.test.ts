...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses VILARO_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      VILARO_HOME: "/srv/vilaro-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/vilaro-home"), ".vilaro", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".vilaro", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("vilaro-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("vilaro-workspace-");
    await fs.mkdir(path.join(tempDir, ".vilaro"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-workspace-hardlink-"));
    try {
