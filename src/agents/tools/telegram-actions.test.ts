...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): VilaroConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as VilaroConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): VilaroConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as VilaroConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: VilaroConfig, accountId = "media") {
    await handleTelegramAction(
