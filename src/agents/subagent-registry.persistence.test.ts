...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["VILARO_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-subagent-"));
  ...
  it("uses isolated temp state when VILARO_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "vilaro-test-state"));
  });
});
