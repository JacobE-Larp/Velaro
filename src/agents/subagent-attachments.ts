...
function resolveAttachmentLimits(config: VilaroConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".vilaro", "attachments");
  const relDir = path.posix.join(".vilaro", "attachments", attachmentId);
