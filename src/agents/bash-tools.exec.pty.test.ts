...
test("exec sets VILARO_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-vilaro-shell", {
    command: "node -e \"process.stdout.write(process.env.VILARO_SHELL || '')\"",
    pty: true,
  });
