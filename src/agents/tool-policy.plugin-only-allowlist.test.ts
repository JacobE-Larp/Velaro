...
const pluginGroups: PluginToolGroups = {
  all: ["vilaro", "workflow_tool"],
  byPlugin: new Map([["vilaro", ["vilaro", "workflow_tool"]]]),
};
...
describe("stripPluginOnlyAllowlist", () => {
  it("strips allowlist when it only targets plugin tools", () => {
    const policy = stripPluginOnlyAllowlist({ allow: ["vilaro"] }, pluginGroups, coreTools);
  ...
  it("keeps allowlist when it mixes plugin and core entries", () => {
    const policy = stripPluginOnlyAllowlist({ allow: ["vilaro", "read"] }, pluginGroups, coreTools);
    expect(policy.policy?.allow).toEqual(["vilaro", "read"]);
    expect(policy.unknownAllowlist).toEqual([]);
  });

  it("strips allowlist with unknown entries when no core tools match", () => {
    const emptyPlugins: PluginToolGroups = { all: [], byPlugin: new Map() };
    const policy = stripPluginOnlyAllowlist({ allow: ["vilaro"] }, emptyPlugins, coreTools);
    expect(policy.policy?.allow).toBeUndefined();
    expect(policy.unknownAllowlist).toEqual(["vilaro"]);
  });

  it("keeps allowlist with core tools and reports unknown entries", () => {
    const emptyPlugins: PluginToolGroups = { all: [], byPlugin: new Map() };
    const policy = stripPluginOnlyAllowlist({ allow: ["read", "vilaro"] }, emptyPlugins, coreTools);
    expect(policy.policy?.allow).toEqual(["read", "vilaro"]);
    expect(policy.unknownAllowlist).toEqual(["vilaro"]);
  });
});
