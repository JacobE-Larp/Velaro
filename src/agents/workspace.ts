...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.VILARO_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".vilaro", `workspace-${profile}`);
  }
  return path.join(home, ".vilaro", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".vilaro";
