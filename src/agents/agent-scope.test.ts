...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: VilaroConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: VilaroConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: VilaroConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: VilaroConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: VilaroConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: VilaroConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/vilaro");
  });

  it("uses VILARO_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "vilaro-home");
    vi.stubEnv("VILARO_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as VilaroConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".vilaro", "workspace"));
  });

  it("uses VILARO_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "vilaro-home");
    vi.stubEnv("VILARO_HOME", home);
    // Clear state dir so it falls back to VILARO_HOME
    vi.stubEnv("VILARO_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as VilaroConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".vilaro", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/vilaro-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/vilaro-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vilaro-agent-scope-"));
