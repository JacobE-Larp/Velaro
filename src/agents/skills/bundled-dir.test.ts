...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["VILARO_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns VILARO_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-bundled-override-"));
    process.env.VILARO_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.VILARO_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "vilaro" }));
