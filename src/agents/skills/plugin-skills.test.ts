    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "vilaro.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "vilaro.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "vilaro.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("vilaro-");
  const acpxRoot = await tempDirs.make("vilaro-acpx-plugin-");
  const helperRoot = await tempDirs.make("vilaro-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("vilaro-");
  const pluginRoot = await tempDirs.make("vilaro-plugin-");
  const outsideDir = await tempDirs.make("vilaro-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("vilaro-");
    const pluginRoot = await tempDirs.make("vilaro-claude-bundle-");
