...
function resolveSkillsLimits(config?: VilaroConfig): ResolvedSkillsLimits {
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: VilaroConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`velaro skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
