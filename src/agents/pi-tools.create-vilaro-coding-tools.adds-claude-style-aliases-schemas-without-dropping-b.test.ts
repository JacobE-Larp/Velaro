...
const defaultTools = createVilaroCodingTools({ senderIsOwner: true });
...
describe("createVilaroCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: VilaroConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createVilaroCodingTools({
    ...
    const anthropicTools = createVilaroCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: VilaroConfig = {
      tools: {
        exec: {
    ...
    const allowed = createVilaroCodingTools({
    ...
    const denied = createVilaroCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createVilaroCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
