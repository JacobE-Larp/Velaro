...
describe("createVilaroCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-read-"));
    try {
      ...
      const contents = "Hello from vilaro read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "vilaro-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "vilaro-workspace"),
    ...
    const tools = createVilaroCodingTools({ sandbox });
