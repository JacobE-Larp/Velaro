...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: VilaroConfig) {
    return createVilaroCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<VilaroConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<VilaroConfig["agents"]>["list"]>[number]["tools"];
  }): VilaroConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createVilaroCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createVilaroCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createVilaroCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createVilaroCodingTools({
      config: cfg,
    ...
    const bobTools = createVilaroCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createVilaroCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createVilaroCodingTools({
      config: cfg,
