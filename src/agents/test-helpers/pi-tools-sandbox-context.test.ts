...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("vilaro-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("vilaro-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("vilaro-sbx-");
