...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.VILARO_GATEWAY_URL) ??
      trimToUndefined(process.env.VILARO_GATEWAY_URL));
  ...
  const allowPrivateWs = process.env.VILARO_ALLOW_INSECURE_PRIVATE_WS === "1";
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__VILARO_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
