...
export function resolveHooksConfig(cfg: VilaroConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: VilaroConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-vilaro-token"] === "string" ? req.headers["x-vilaro-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-vilaro-idempotency-key"]) ||
