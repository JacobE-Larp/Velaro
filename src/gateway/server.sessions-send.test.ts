...
const { createVilaroTools } = await import("../agents/vilaro-tools.js");
...
type SessionSendTool = ReturnType<typeof createVilaroTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createVilaroTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["VILARO_GATEWAY_PORT", "VILARO_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.VILARO_GATEWAY_PORT = String(gatewayPort);
  process.env.VILARO_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("VILARO_CONFIG_PATH missing in gateway test environment");
      }
