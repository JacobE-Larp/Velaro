...
function listConfiguredAgentIds(cfg: VilaroConfig): string[] {
...
export function listAgentsForGateway(cfg: VilaroConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: VilaroConfig): string {
  return normalizeAgentId(resolveDefaultAgentId(cfg));
}

export function resolveSessionStoreKey(params: { cfg: VilaroConfig; sessionKey: string }): string {
...
function resolveSessionStoreAgentId(cfg: VilaroConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: VilaroConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: VilaroConfig): GatewaySessionsDefaults {
