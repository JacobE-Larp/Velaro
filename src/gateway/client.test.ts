...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("vilaro doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with VILARO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with VILARO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.VILARO_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://vilaro-gateway.ai:18789",
      onConnectError,
    });
