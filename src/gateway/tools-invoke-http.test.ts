...
let lastCreateVilaroToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/vilaro-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateVilaroToolsContext?.agentTo,
          agentThreadId: lastCreateVilaroToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createVilaroTools: (ctx: Record<string, unknown>) => {
      lastCreateVilaroToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateVilaroToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateVilaroToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-vilaro-message-to": "channel:24514",
        "x-vilaro-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
