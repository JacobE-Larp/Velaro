...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "VILARO_STATE_DIR",
  "VILARO_CONFIG_PATH",
  "VILARO_SKIP_BROWSER_CONTROL_SERVER",
  "VILARO_SKIP_GMAIL_WATCHER",
  "VILARO_SKIP_CANVAS_HOST",
  "VILARO_BUNDLED_PLUGINS_DIR",
  "VILARO_SKIP_CHANNELS",
  "VILARO_SKIP_PROVIDERS",
  "VILARO_SKIP_CRON",
  "VILARO_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.VILARO_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.VILARO_SKIP_GMAIL_WATCHER = "1";
  process.env.VILARO_SKIP_CANVAS_HOST = "1";
  process.env.VILARO_SKIP_CHANNELS = "1";
  process.env.VILARO_SKIP_PROVIDERS = "1";
  process.env.VILARO_SKIP_CRON = "1";
  process.env.VILARO_TEST_MINIMAL_GATEWAY = "1";
  process.env.VILARO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "vilaro-test-no-bundled-extensions")
    : "vilaro-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".vilaro-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".vilaro-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__vilaroTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__vilaroTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["VILARO_GATEWAY_TOKEN"]);
  const prev = process.env.VILARO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.VILARO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.VILARO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
