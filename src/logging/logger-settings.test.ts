...
let originalVilaroLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.VILARO_TEST_FILE_LOG;
  originalVilaroLogLevel = process.env.VILARO_LOG_LEVEL;
  delete process.env.VILARO_TEST_FILE_LOG;
  delete process.env.VILARO_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.VILARO_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalVilaroLogLevel === undefined) {
    delete process.env.VILARO_LOG_LEVEL;
  } else {
    process.env.VILARO_LOG_LEVEL = originalVilaroLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
