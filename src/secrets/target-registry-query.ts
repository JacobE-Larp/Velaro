...
const VILARO_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of VILARO_COMPILED_SECRET_TARGETS) {
...
const VILARO_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargets(config: VilaroConfig): DiscoveredConfigSecretTarget[] {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: VILARO_COMPILED_SECRET_TARGETS,
    entriesById: VILARO_TARGETS_BY_ID,
  });
