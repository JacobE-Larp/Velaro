...
export function isCommandEnabled(cfg: VilaroConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: VilaroConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: VilaroConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: VilaroConfig) {
