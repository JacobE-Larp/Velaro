...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop vilaro!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@vilaro_bot", { botUsername: "vilaro_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@vilaro_bot", { botUsername: "vilaro_bot" })).toBe(true);
