...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: VilaroConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: VilaroConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/vilaro-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<VilaroConfig["bindings"]>[number];
