...
function listAgents(cfg: VilaroConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: VilaroConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<VilaroConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: VilaroConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: VilaroConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: VilaroConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<VilaroConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  VilaroConfig,
  {
    bindingsRef: VilaroConfig["bindings"];
    agentsRef: VilaroConfig["agents"];
    sessionRef: VilaroConfig["session"];
...
function resolveRouteCacheForConfig(cfg: VilaroConfig): Map<string, ResolvedAgentRoute> {
