...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      vilaro: { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vilaro-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "vilaro.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.vilaro/extensions and preserves scoped package ids", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@vilaro/voice-call" });
  });
  ...
  it("installs from a zip archive", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@vilaro/zipper" });
  });
  ...
  it("rejects packages without vilaro.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@vilaro/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("vilaro.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_VILARO_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when vilaro.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@vilaro/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing vilaro.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_VILARO_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without vilaro.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.vilaro).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses vilaro.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@vilaro/cognee-vilaro"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it("preserves scoped package names when no plugin manifest id is present", async () => {
    ...
    expectInstalledWithPluginId(res, extensionsDir, "@vilaro/test-plugin");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@vilaro/voice-call@0.0.1",
              name: "@vilaro/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@vilaro/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@vilaro/voice-call@0.0.1",
    });
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@vilaro/voice-call@0.0.2-beta.1",
      name: "@vilaro/voice-call",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@vilaro/voice-call@beta"');
    }
  });
  ...
  it("allows explicit prerelease npm tags", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@vilaro/voice-call@0.0.2-beta.1");
    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@vilaro/voice-call@beta",
    });
    expect(packTmpDir).not.toBe("");
  });
});
