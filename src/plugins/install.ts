...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing vilaro.extensions; update the plugin package to include vilaro.extensions (for example ["./dist/index.js"]). See https://docs.vilaro.ai/help/troubleshooting#plugin-install-fails-with-missing-vilaro-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_VILARO_EXTENSIONS: "missing_vilaro_extensions",
  EMPTY_VILARO_EXTENSIONS: "empty_vilaro_extensions",
...
function ensureVilaroExtensions(params: { manifest: PackageManifest }):
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_VILARO_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json vilaro.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_VILARO_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  // Prefer the canonical `id` from vilaro.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-vilaro"), the plugin registry
