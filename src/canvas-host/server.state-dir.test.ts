...
describe("canvas host state dir defaults", () => {
  it("uses VILARO_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("vilaro-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Vilaro Canvas");
      } finally {
        await handler.close();
      }
    });
