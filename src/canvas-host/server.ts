...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.vilaroCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.vilaroCanvasA2UIAction &&
        typeof window.vilaroCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.vilaroSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("vilaro:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Vilaro node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.vilaroSendUserAction === "function"
        ? window.vilaroSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.VILARO_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.VILARO_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Vilaro Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
