...
describe("resolveInitialTuiAgentId", () => {
  const cfg: VilaroConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/vilaro" },
        { id: "ops", workspace: "/tmp/vilaro/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/vilaro/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run vilaro devices list");
    expect(state.pairingHint).toContain("vilaro devices list");
  });
