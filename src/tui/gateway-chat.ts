...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set VILARO_GATEWAY_TOKEN/VILARO_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.VILARO_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.VILARO_GATEWAY_PASSWORD);
