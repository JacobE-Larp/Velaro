...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: VilaroConfig) => Array<string | number> | undefined;
}): VilaroConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: VilaroConfig) => VilaroConfig | Promise<VilaroConfig>;
  applySet?: (
    cfg: VilaroConfig,
    value: SecretInput,
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in Velaro config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "VILARO_SECRET",
      envVarFormatError:
        'Use an env var name like "VILARO_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
