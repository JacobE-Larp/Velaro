...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: VilaroConfig, policy: ChannelAccessPolicy) => VilaroConfig;
  resolveAllowlist: (params: { cfg: VilaroConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: VilaroConfig; resolved: TResolved }) => VilaroConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: VilaroConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: VilaroConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: VilaroConfig, policy: ChannelAccessPolicy): VilaroConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: VilaroConfig, policy: ChannelAccessPolicy): VilaroConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: VilaroConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: VilaroConfig; resolved: string[] }) => {
      calls.push("apply");
