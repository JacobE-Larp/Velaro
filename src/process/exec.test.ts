...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        VILARO_BASE_ENV: "base",
        VILARO_TO_REMOVE: undefined,
      },
      env: {
        VILARO_TEST_ENV: "ok",
      },
    ...
    expect(resolved.VILARO_BASE_ENV).toBe("base");
    expect(resolved.VILARO_TEST_ENV).toBe("ok");
    expect(resolved.VILARO_TO_REMOVE).toBeUndefined();
    expect(resolved.VILARO_CLI).toBe(VILARO_CLI_ENV_VALUE);
  });
