...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".vilaro");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.VILARO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$VILARO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via VILARO_STATE_DIR.
