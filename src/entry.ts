...
import { ensureVilaroExecMarkerOnProcess } from "./infra/vilaro-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "vilaro.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "vilaro.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.VILARO_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.VILARO_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.VILARO_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.VILARO_NODE_OPTIONS_READY = "1";
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      ...
      .catch((error) => {
        console.error(
          "[vilaro] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[vilaro] ${parsed.error}`);
      process.exit(2);
    }
