...
type StopChromeTarget = Parameters<typeof stopVilaroChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "vilaro-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateVilaroProfile(userDataDir, { color: DEFAULT_VILARO_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0xff4500) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_VILARO_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_VILARO_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(path.join(userDataDir, ".vilaro-profile-decorated"), "utf-8");
    expect(marker.trim()).toMatch(/^\d+$/);
  });

  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateVilaroProfile(userDataDir, { color: "vilaro-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_VILARO_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopVilaroChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopVilaroChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopVilaroChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
