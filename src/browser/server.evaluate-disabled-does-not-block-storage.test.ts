  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          vilaro: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.VILARO_GATEWAY_PORT;
    process.env.VILARO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.VILARO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.VILARO_GATEWAY_PASSWORD;
