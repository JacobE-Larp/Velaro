...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.VILARO_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "vilaro",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/vilaro" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    vilaro: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "vilaro";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.VILARO_GATEWAY_PORT;
  process.env.VILARO_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.VILARO_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.VILARO_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.VILARO_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.VILARO_GATEWAY_PASSWORD;
  } else {
    process.env.VILARO_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
