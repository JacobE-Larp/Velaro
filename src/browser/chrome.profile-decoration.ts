...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".vilaro-profile-decorated");
}
...
export function decorateVelaroProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_VILARO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_VILARO_BROWSER_COLOR).toUpperCase();
