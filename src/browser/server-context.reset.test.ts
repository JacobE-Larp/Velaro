...
function localVilaroProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "vilaro",
...
function createLocalVilaroResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localVilaroProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveVilaroUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vilaro-reset-"));
    const profileDir = path.join(tempRoot, "vilaro");
    ...
    const ops = createLocalVilaroResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveVilaroUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vilaro-reset-no-own-"));
    const profileDir = path.join(tempRoot, "vilaro");
    fs.mkdirSync(profileDir, { recursive: true });
