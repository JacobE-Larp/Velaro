...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredVilaroTmpDir: vi.fn(() => "/tmp/vilaro"),
}));
vi.mock("../infra/tmp-vilaro-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredVilaroTmpDir.mockReturnValue("/tmp/vilaro");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".vilaro-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredVilaroTmpDir.mockReturnValue("/tmp/vilaro-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "vilaro-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "vilaro-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredVilaroTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "vilaro-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "vilaro-preferred", "downloads")}${path.sep}`),
    );
  });
