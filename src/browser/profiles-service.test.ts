...
vi.mock("./chrome.js", () => ({
  resolveVilaroUserDataDir: vi.fn(() => "/tmp/vilaro-test/vilaro/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "vilaro-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveVilaroUserDataDir).mockReturnValue(userDataDir);
