...
const { stopVilaroChromeMock } = vi.hoisted(() => ({
  stopVilaroChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopVilaroChrome: stopVilaroChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopVilaroChromeMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["vilaro", "user"]);
    ...
    expect(stopMap.vilaro).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopVilaroChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("vilaro browser stop failed: Error: oops");
  });
});
