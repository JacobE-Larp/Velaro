...
const createTempDir = () => tempDirs.make("vilaro-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets VILARO_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "vilaro",
    });

    expect(env.VILARO_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("vilaro");
  });

  it("overrides pre-existing VILARO_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      VILARO_SHELL: "wrong",
    });
    expect(env.VILARO_SHELL).toBe("acp-client");
  ...
  it("preserves VILARO_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        VILARO_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["VILARO_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Vilaro bridge", () => {
    ...
    expect(env.VILARO_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.VILARO_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "vilaro",
        serverArgs: ["acp"],
        defaultServerCommand: "vilaro",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("VILARO_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "vilaro", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "vilaro", "dist", "entry.js");
    const shimPath = path.join(dir, "vilaro.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\vilaro\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/vilaro-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/vilaro-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/vilaro-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
