...
`;
    ...
    expect(parsed.vilaro.emoji).toBe("💾");
    expect(parsed.vilaro.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.vilaro.emoji).toBe("📝");
    expect(parsed.vilaro.events).toEqual(["command"]);
    expect(parsed.vilaro.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.vilaro.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"vilaro": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"vilaro": {"events": ["test"]}}');
  });
...
describe("resolveVilaroMetadata", () => {
  it("extracts vilaro metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveVilaroMetadata(frontmatter);
  ...
  it("returns undefined when vilaro key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        vilaro: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Vilaro" },
            { id: "npm", kind: "npm", package: "@vilaro/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@vilaro/hook");
  });
    ...
    "vilaro":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Vilaro" }],
      },
  }
---
...
`;
    ...
    const vilaro = resolveVilaroMetadata(frontmatter);
    expect(vilaro).toBeDefined();
    expect(vilaro?.emoji).toBe("💾");
    expect(vilaro?.events).toEqual(["command:new", "command:reset"]);
    expect(vilaro?.requires?.config).toEqual(["workspace.dir"]);
    expect(vilaro?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const vilaro = resolveVilaroMetadata(frontmatter);
    expect(vilaro?.emoji).toBe("disk");
    expect(vilaro?.events).toEqual(["command:new"]);
  });
});
