...
This directory contains hooks that ship with Vilaro. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.vilaro/vilaro.json`:
...
2. Restart gateway: `pkill -9 -f 'vilaro.*gateway' && pnpm vilaro gateway`
3. Enable the hook: `vilaro hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.vilaro.ai/automation/hooks
