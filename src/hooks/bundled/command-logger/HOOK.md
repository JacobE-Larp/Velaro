...
homepage: https://docs.vilaro.ai/automation/hooks#command-logger
metadata:
  {
    "vilaro":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.vilaro/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.vilaro/logs/commands.log ~/.vilaro/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/vilaro`:
   ```
   /home/username/.vilaro/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
