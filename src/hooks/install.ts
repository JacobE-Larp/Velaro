...
async function ensureVilaroHooks(manifest: HookPackageManifest) {
  ...
  if (list.length === 0) {
    throw new Error("package.json vilaro.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  for (const entry of hookEntries) {
    ...
    if (!isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `vilaro.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `vilaro.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await installFromValidatedNpmSpecArchive({
    tempDirPrefix: "vilaro-hook-pack-",
    spec,
    timeoutMs,
