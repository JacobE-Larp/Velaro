...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "vilaro-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "vilaro-bundled-skills-test-"));
    process.env.VILARO_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
