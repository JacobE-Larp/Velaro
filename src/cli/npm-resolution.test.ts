...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@vilaro/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@vilaro/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@vilaro/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@vilaro/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @vilaro/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@vilaro/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@vilaro/plugin-alpha@1.2.3",
        installPath: "/tmp/vilaro/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@vilaro/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @vilaro/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@vilaro/plugin-alpha@latest",
      true,
      "/tmp/vilaro/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@vilaro/plugin-alpha@latest",
      installPath: "/tmp/vilaro/extensions/alpha",
      version: "1.2.3",
