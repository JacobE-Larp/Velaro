...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("VILARO_GATEWAY_TOKEN", "");
    vi.stubEnv("VILARO_GATEWAY_TOKEN", "");
    vi.stubEnv("VILARO_GATEWAY_PASSWORD", "");
    vi.stubEnv("VILARO_GATEWAY_PASSWORD", "");
  });
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("vilaro devices approve <requestId>");
  });
  ...
  it("uses VILARO_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("VILARO_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
