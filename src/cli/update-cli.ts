...
${theme.heading("Switch channels:")}
  ...
  - Run vilaro update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.vilaro.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.vilaro.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
