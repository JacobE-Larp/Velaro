...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "vilaro";
...
function applySlotSelectionForPlugin(
  config: VilaroConfig,
  pluginId: string,
): { config: VilaroConfig; warnings: string[] } {
...
async function runPluginInstallCommand(params: {
  ...
  if (fs.existsSync(resolved)) {
    if (opts.link) {
      ...
      let next: VilaroConfig = enablePluginInConfig(
        {
          ...cfg,
          plugins: {
            ...cfg.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.vilaro.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action((id: string, opts: PluginInfoOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${plugin.format ?? "vilaro"}`);
      if (plugin.bundleFormat) {
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: VilaroConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.vilaro.ai/plugin");
      lines.push("");
