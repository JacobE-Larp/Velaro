...
beforeEach(() => {
  ...
  originalHideBanner = process.env.VILARO_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.VILARO_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.VILARO_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("vilaro");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "vilaro", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("vilaro-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "vilaro", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "vilaro", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vilaro", "channels", "add"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "vilaro", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.VILARO_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "vilaro", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "vilaro", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vilaro", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "vilaro", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "vilaro", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "vilaro", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "vilaro", "backup", "create", "--json"],
    });
