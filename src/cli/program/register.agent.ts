...
${formatHelpExamples([
  ['vilaro agent --to +15555550123 --message "status update"', "Start a new session."],
  ['vilaro agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['vilaro agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.vilaro.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.vilaro.ai/cli/agents")}\n`,
    );
