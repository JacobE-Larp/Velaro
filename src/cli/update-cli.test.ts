...
import type { VilaroConfig, ConfigFileSnapshot } from "../config/types.vilaro.js";
...
vi.mock("../infra/vilaro-root.js", () => ({
  resolveVilaroPackageRoot: vi.fn(),
}));
...
const { resolveVilaroPackageRoot } = await import("../infra/vilaro-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/vilaro-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as VilaroConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/vilaro-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveVilaroPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("vilaro-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveVilaroPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("vilaro");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/vilaro-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "vilaro-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Vilaro update status");
  });
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as VilaroConfig,
        });
      },
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "vilaro@latest", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("honors --tag override", async () => {
    ...
    expect(runGatewayUpdate).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "vilaro@next", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("vilaro-update");
    const localAppData = createCaseDir("vilaro-localappdata");
  ...
  it("uses VILARO_UPDATE_PACKAGE_SPEC for package updates", async () => {
    const tempDir = createCaseDir("vilaro-update");
    mockPackageInstallStatus(tempDir);
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        ...
        "i",
        "-g",
        "http://10.211.55.2:8138/vilaro-next.tgz",
        "--no-fund",
        "--no-audit",
        "--loglevel=error",
      ],
  ...
  it("passes explicit git package specs through for package updates", async () => {
    ...
    await updateCommand({ yes: true, tag: "github:vilaro/vilaro#main" });
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("vilaro-updated-root");
    ...
    await withEnvAsync(
      {
        ...
        VILARO_CONFIG_PATH: "./config/vilaro.json",
      },
      async () => {
        await updateCommand({});
      },
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          VILARO_STATE_DIR: path.resolve("./state"),
          VILARO_CONFIG_PATH: path.resolve("./config/vilaro.json"),
        }),
        timeoutMs: 60_000,
      }),
    );
  ...
  it("updateCommand reuses the captured invocation cwd when process.cwd later fails", async () => {
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: root,
        env: expect.objectContaining({
          VILARO_STATE_DIR: path.resolve(originalCwd, "./state"),
        }),
        timeoutMs: 60_000,
      }),
    );
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ VILARO_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.VILARO_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("vilaro-update-wizard");
    await withEnvAsync({ VILARO_GIT_DIR: tempDir }, async () => {
      setTty(true);
