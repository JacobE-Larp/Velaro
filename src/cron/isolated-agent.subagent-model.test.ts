...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "vilaro-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".vilaro", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<VilaroConfig> = {},
): VilaroConfig {
  const base: VilaroConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<VilaroConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<VilaroConfig>,
