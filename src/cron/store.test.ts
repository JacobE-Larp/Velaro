...
const { makeStorePath } = createCronStoreHarness({ prefix: "vilaro-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses VILARO_HOME for tilde expansion", () => {
    vi.stubEnv("VILARO_HOME", "/srv/vilaro-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/vilaro-home", "cron", "jobs.json"));
  });
});
