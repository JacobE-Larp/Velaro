...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: VilaroConfig | null = null;
  ...
  get(key: string, cfg: VilaroConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: VilaroConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: VilaroConfig): void {
