...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Vilaro tmp root", async () => {
      const tmpRoot = resolvePreferredVilaroTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideVilaro = path.join(os.tmpdir(), "outside-vilaro", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
