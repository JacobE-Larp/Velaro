...
let ensureVilaroCliOnPath: typeof import("./path-env.js").ensureVilaroCliOnPath;

describe("ensureVilaroCliOnPath", () => {
  const envKeys = [
    "PATH",
    "VILARO_PATH_BOOTSTRAPPED",
    "VILARO_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureVilaroCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/vilaro-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "vilaro");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureVilaroCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling vilaro exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.VILARO_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.VILARO_PATH_BOOTSTRAPPED = "1";
    ensureVilaroCliOnPath({
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "vilaro");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.VILARO_PATH_BOOTSTRAPPED;
      delete process.env.VILARO_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.VILARO_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/vilaro-path/case-linuxbrew");
