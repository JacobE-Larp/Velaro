...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["VILARO_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.VILARO_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveVilaroPackageRoot } = await import("./vilaro-root.js"));
    ...
    vi.mocked(resolveVilaroPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveVilaroPackageRoot).mockResolvedValue("/opt/vilaro");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/vilaro/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
