...
const DEFAULT_PACKAGE_NAME = "vilaro";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["vilaro", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "vilaro.mjs");
    ...
    const doctorStep = await runStep(
      step("vilaro doctor", doctorArgv, gitRoot, { VILARO_UPDATE_IN_PROGRESS: "1" }),
    );
