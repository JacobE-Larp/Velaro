...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__vilaro_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "vilaro-root");
...
describe("resolveVilaroPackageRoot", () => {
  let resolveVilaroPackageRoot: typeof import("./vilaro-root.js").resolveVilaroPackageRoot;
  let resolveVilaroPackageRootSync: typeof import("./vilaro-root.js").resolveVilaroPackageRootSync;

  beforeAll(async () => {
    ({ resolveVilaroPackageRoot, resolveVilaroPackageRootSync } = await import("./vilaro-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "vilaro");
    const pkgRoot = path.join(project, "node_modules", "vilaro");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "vilaro" }));

    expect(resolveVilaroPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "vilaro");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "vilaro.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "vilaro" }));

    expect(resolveVilaroPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveVilaroPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("falls through from a non-vilaro moduleUrl candidate to cwd", async () => {
    const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
    const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
    setFile(path.join(wrongPkgRoot, "package.json"), JSON.stringify({ name: "not-vilaro" }));
    setFile(path.join(cwdPkgRoot, "package.json"), JSON.stringify({ name: "vilaro" }));
    const moduleUrl = pathToFileURL(path.join(wrongPkgRoot, "dist", "index.js")).toString();

    expect(resolveVilaroPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveVilaroPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveVilaroPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveVilaroPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-vilaro package roots", async () => {
    const pkgRoot = fx("not-vilaro");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-vilaro" }));

    expect(resolveVilaroPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("falls back from a symlinked argv1 to the node_modules package root", () => {
    const project = fx("symlink-node-modules-fallback");
    const argv1 = path.join(project, "node_modules", ".bin", "vilaro");
    state.realpaths.set(abs(argv1), abs(path.join(project, "versions", "current", "vilaro.mjs")));
    const pkgRoot = path.join(project, "node_modules", "vilaro");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "vilaro" }));
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveVilaroPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveVilaroPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
