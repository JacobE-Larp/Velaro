 ...
 * returning control to the caller (typically `triggerVilaroRestart` →
...
/**
 * Parse vilaro gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("vilaro")) {
 ...
 * Returns only PIDs that belong to vilaro gateway processes (not the current process).
 */
