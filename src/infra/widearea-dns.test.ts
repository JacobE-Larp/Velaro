...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "vilaro.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Vilaro)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "vilaro.internal", expected: "vilaro.internal." },
    { value: "vilaro.internal.", expected: "vilaro.internal." },
    { value: "  vilaro.internal  ", expected: "vilaro.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { VILARO_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { VILARO_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("vilaro.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "vilaro.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      instanceLabel: "studio-london",
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/vilaro",
    });

    expect(txt).toContain(`$ORIGIN vilaro.internal.`);
    ...
    expect(txt).toContain(`_vilaro-gw._tcp IN PTR studio-london._vilaro-gw._tcp`);
    expect(txt).toContain(`studio-london._vilaro-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Vilaro)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/vilaro`);
  });
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN vilaro.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._vilaro-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (Vilaro)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("vilaro.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("vilaro.internal."),
