...
export function resolvePreferredVilaroTmpDir(
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[vilaro] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const existingPreferredState = resolveDirState(POSIX_VILARO_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_VILARO_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_VILARO_TMP_DIR)) {
      return POSIX_VILARO_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_VILARO_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_VILARO_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_VILARO_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_VILARO_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_VILARO_TMP_DIR;
