...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.VILARO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*vilaro-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "vilaro";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "vilaro");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "vilaro-gw");
  ...
  records.push(`_vilaro-gw._tcp IN PTR ${instanceLabel}._vilaro-gw._tcp`);
  records.push(`${instanceLabel}._vilaro-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._vilaro-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; vilaro-content-hash: ${contentHash}\n${contentBody}`;
}
