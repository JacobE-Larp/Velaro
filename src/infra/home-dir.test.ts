...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers VILARO_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for VILARO_HOME", () => {
    const result = resolveRequiredHomeDir(
      { VILARO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when VILARO_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ VILARO_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { VILARO_HOME: "/srv/vilaro-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/vilaro-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/vilaro-home " },
      expected: "/srv/vilaro-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/vilaro-home/docs"));
  });
