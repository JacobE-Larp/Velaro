...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevVilaroHome = process.env.VILARO_HOME;
    const prevHome = process.env.HOME;
    process.env.VILARO_HOME = "/srv/vilaro-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/vilaro-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevVilaroHome === undefined) {
        delete process.env.VILARO_HOME;
      } else {
        process.env.VILARO_HOME = prevVilaroHome;
      }
