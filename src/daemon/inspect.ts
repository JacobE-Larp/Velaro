...
const EXTRA_MARKERS = ["vilaro", "vilaro", "vilaro"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["vilaro_service_marker"];
  const kindKeys = ["vilaro_service_kind"];
...
function isVelaroGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("vilaro-gateway")) {
    return false;
  }
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("vilaro") || lower.includes("vilaro");
}
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "vilaro" : "vilaro",
        legacy: true,
      });
      continue;
    }
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "vilaro" || isLegacyLabel(label),
    });
  }
