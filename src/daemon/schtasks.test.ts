...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when VILARO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".vilaro", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when VILARO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", VILARO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".vilaro-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers VILARO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        VILARO_PROFILE: "rescue",
        VILARO_STATE_DIR: "C:\\State\\vilaro",
      },
      expected: path.join("C:\\State\\vilaro", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", VILARO_PROFILE: "default" },
      expected: path.join("/home/test", ".vilaro", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Vilaro Gateway",
          "cd /d C:\\Projects\\vilaro",
          "set NODE_ENV=production",
          "set VILARO_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\vilaro",
          environment: {
            NODE_ENV: "production",
            VILARO_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\vilaro\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\vilaro\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Vilaro Share\\node.exe" "\\\\fileserver\\Vilaro Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Vilaro Share\\node.exe",
            "\\\\fileserver\\Vilaro Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from VILARO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ VILARO_STATE_DIR: path.join(tmpDir, "custom-state") }),
