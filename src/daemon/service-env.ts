...
export function buildServiceEnvironment(params: {
  ...
  return {
    ...
    VILARO_PROFILE: profile,
    VILARO_GATEWAY_PORT: String(port),
    VILARO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    VILARO_SYSTEMD_UNIT: systemdUnit,
    VILARO_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    VILARO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    VILARO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.VILARO_GATEWAY_TOKEN?.trim() || env.VILARO_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    VILARO_GATEWAY_TOKEN: gatewayToken,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    VILARO_STATE_DIR: sharedEnv.stateDir,
    VILARO_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.VILARO_STATE_DIR;
  const configPath = env.VILARO_CONFIG_PATH;
