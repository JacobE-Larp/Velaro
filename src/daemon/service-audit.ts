...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds VILARO_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `velaro gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service VILARO_GATEWAY_TOKEN does not match gateway.auth.token in vilaro.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.VILARO_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.VILARO_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `velaro gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
