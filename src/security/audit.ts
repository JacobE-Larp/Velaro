...
import type { ConfigFileSnapshot, VilaroConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: VilaroConfig): boolean {
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.VILARO_GATEWAY_TOKEN) || hasNonEmptyString(env.VILARO_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.VILARO_GATEWAY_PASSWORD) ||
    hasNonEmptyString(env.VILARO_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.VILARO_GATEWAY_TOKEN) ||
    hasNonEmptyString(env.VILARO_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: VilaroConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: VilaroConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: VilaroConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
