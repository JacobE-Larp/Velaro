...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: VilaroConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: VilaroConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: VilaroConfig) => string[];
  isConfigured?: (account: unknown, cfg: VilaroConfig) => boolean;
  isEnabled?: (account: unknown, cfg: VilaroConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "vilaro.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ VILARO_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        vilaro: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.VILARO_GATEWAY_TOKEN;
    const prevPassword = process.env.VILARO_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.VILARO_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.VILARO_GATEWAY_PASSWORD;
      } else {
        process.env.VILARO_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("does not flag missing gateway auth when read-only scrubbed config omits unavailable auth SecretRefs", async () => {
    const sourceConfig: VilaroConfig = {
      gateway: {
        bind: "lan",
        auth: {
          token: {
    ...
    const resolvedConfig: VilaroConfig = {
      gateway: {
        bind: "lan",
        auth: {},
      },
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/vilaro-safe-bins"];
    const cfg: VilaroConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("vilaro-sbx-browser-old\nvilaro-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "vilaro-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "vilaro-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("vilaro-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("vilaro-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "vilaro-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "vilaro-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-vilaro.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: VilaroConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: VilaroConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): VilaroConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain("~/.vilaro/credentials/discord-allowFrom.json:team.owner");
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: VilaroConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.VILARO_GATEWAY_TOKEN;
    process.env.VILARO_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: VilaroConfig = {
  ...
  it("scores unrestricted hooks.allowedAgentIds by gateway exposure", async () => {
    ...
    } satisfies NonNullable<VilaroConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: VilaroConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.vilaro",
      configPath: "/Users/test/Dropbox/.vilaro/vilaro.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: VilaroConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "vilaro.json"),
        execDockerRawFn: execDockerRawUnavailable,
      });
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "vilaro.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@vilaro/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@vilaro/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.VILARO_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.VILARO_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
