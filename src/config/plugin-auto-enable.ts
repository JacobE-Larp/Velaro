...
function isWhatsAppConfigured(cfg: VilaroConfig): boolean {
...
function isGenericChannelConfigured(cfg: VilaroConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: VilaroConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: VilaroConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: VilaroConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: VilaroConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: VilaroConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: VilaroConfig, pluginId: string): VilaroConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
