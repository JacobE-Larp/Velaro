  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.vilaro/logs/vilaro.log" },
        hooks: {
          path: "~/.vilaro/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.vilaro/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".vilaro", "logs", "vilaro.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".vilaro", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".vilaro", "telegram.token"),
      );
