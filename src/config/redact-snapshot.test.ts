...
import type { ConfigFileSnapshot } from "./types.vilaro.js";
import { VilaroSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(VilaroSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.vilaro/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/vilaro/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/vilaro/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/vilaro/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/vilaro/nickserv-password.txt");
