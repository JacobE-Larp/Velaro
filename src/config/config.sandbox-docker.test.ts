...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("vilaro-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "vilaro-sandbox-browser-global" },
      agentBrowser: { network: "vilaro-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("vilaro-sandbox-browser-agent");
  });
