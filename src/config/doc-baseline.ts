...
function resolveRepoRoot(): string {
  const fromPackage = resolveVilaroPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    VILARO_STATE_DIR: path.join(os.tmpdir(), "vilaro-config-doc-baseline-state"),
    VILARO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
