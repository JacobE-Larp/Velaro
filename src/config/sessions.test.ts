...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ VILARO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-vilaro",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-vilaro#general");
  });
  ...
  it("derives session transcripts dir from VILARO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { VILARO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when VILARO_STATE_DIR differs from stored paths", () => {
