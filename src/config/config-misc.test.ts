...
import { buildWebSearchProviderConfig, withTempHome, writeVilaroConfig } from "./test-helpers.js";
import { VilaroSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = VilaroSchema.safeParse({
      $schema: "https://vilaro.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://vilaro.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = VilaroSchema.safeParse({ $schema: 123 });
...
describe("ui.seamColor", () => {
  ...
  it("rejects non-hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "vilaro" } });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = VilaroSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeVilaroConfig(home, {
        gateway: { bind: "${VILARO_BIND}" },
      });

      const prev = process.env.VILARO_BIND;
      process.env.VILARO_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.VILARO_BIND;
        } else {
          process.env.VILARO_BIND = prev;
        }
      }
    });
  });
