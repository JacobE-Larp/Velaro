...
import type { VilaroConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function unsetPathForWrite(
  root: VilaroConfig,
  pathSegments: string[],
): { changed: boolean; next: VilaroConfig } {
...
function coerceConfig(value: unknown): VilaroConfig {
...
function stampConfigVersion(cfg: VilaroConfig): VilaroConfig {
...
function warnIfConfigFromFuture(cfg: VilaroConfig, logger: Pick<typeof console, "warn">): void {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as VilaroConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): VilaroConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as VilaroConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: VilaroConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as VilaroConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.VILARO_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.VILARO_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.VILARO_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.VILARO_WATCH_SESSION === "string" &&
        deps.env.VILARO_WATCH_SESSION.trim().length > 0
          ? deps.env.VILARO_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.VILARO_WATCH_COMMAND === "string" &&
        deps.env.VILARO_WATCH_COMMAND.trim().length > 0
          ? deps.env.VILARO_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `VILARO_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: VilaroConfig | null = null;
let runtimeConfigSourceSnapshot: VilaroConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.VILARO_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.VILARO_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function setRuntimeConfigSnapshot(config: VilaroConfig, sourceConfig?: VilaroConfig): void {
...
export function getRuntimeConfigSnapshot(): VilaroConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): VilaroConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: VilaroConfig;
  candidate: VilaroConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: VilaroConfig): VilaroConfig {
...
export function loadConfig(): VilaroConfig {
...
export async function readBestEffortConfig(): Promise<VilaroConfig> {
...
export async function writeConfigFile(
  ...
  // edits to vilaro.json remain visible (no stale snapshot).
}
