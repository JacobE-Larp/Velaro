...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "vilaro.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.vilaro/workspace/canvas). */
  root?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/vilaro"). */
  basePath?: string;
 ...
 * Used when Velaro runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
...
export type GatewayReloadConfig = {
   ...
   * @see https://github.com/vilaro/vilaro/issues/47711
   */
