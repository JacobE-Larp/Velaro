...
function validateIdentityAvatar(config: VilaroConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: VilaroConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: VilaroConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as VilaroConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as VilaroConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as VilaroConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as VilaroConfig,
  };
}
