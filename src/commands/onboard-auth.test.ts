...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under VILARO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("vilaro-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-oauth-sync-"));
    ...
    process.env.VILARO_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-oauth-nosync-"));
    process.env.VILARO_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside VILARO_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-oauth-external-"));
    process.env.VILARO_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* VILARO_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to VILARO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("vilaro-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
