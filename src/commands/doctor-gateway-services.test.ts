...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.VILARO_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.VILARO_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      VILARO_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service VILARO_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: VilaroConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.VILARO_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("uses VILARO_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ VILARO_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/vilaro/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/vilaro@2026.3.12/node_modules/vilaro/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    fsMocks.realpath.mockImplementation(async (value: string) => {
      if (value.includes("/global/5/node_modules/vilaro/")) {
        return value.replace(
          "/global/5/node_modules/vilaro/",
          "/global/5/node_modules/.pnpm/vilaro@2026.3.12/node_modules/vilaro/",
        );
      }
      return value;
    });
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/opt/vilaro/../vilaro/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/vilaro/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            VILARO_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    mocks.findExtraGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "vilaro-gateway.service",
        detail: "unit: /home/test/.config/systemd/user/vilaro-gateway.service",
        scope: "user",
        legacy: true,
      },
    ]);
    mocks.uninstallLegacySystemdUnits.mockResolvedValue([
      {
        name: "vilaro-gateway",
        unitPath: "/home/test/.config/systemd/user/vilaro-gateway.service",
        enabled: true,
        exists: true,
      },
    ]);
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      expect.stringContaining("vilaro-gateway.service"),
      "Legacy gateway removed",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing Vilaro gateway next.",
    );
  });
});
