...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.VILARO_GATEWAY_TOKEN ?? process.env.VILARO_GATEWAY_TOKEN ?? configuredToken;
  const password =
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from vilaro.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const baseConfig: VilaroConfig = snapshot.valid ? snapshot.config : {};
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token:
        process.env.VILARO_GATEWAY_TOKEN ?? process.env.VILARO_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password:
        process.env.VILARO_GATEWAY_PASSWORD ??
        process.env.VILARO_GATEWAY_PASSWORD ??
