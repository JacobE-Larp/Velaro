...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "VILARO_GATEWAY_TOKEN",
          placeholder: "VILARO_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like VILARO_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Velaro will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Velaro trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.vilaro.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
