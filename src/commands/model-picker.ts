...
type PromptDefaultModelResult = { model?: string; config?: VilaroConfig };
...
function resolveConfiguredModelRaw(cfg: VilaroConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: VilaroConfig): string[] {
...
export function applyPrimaryModel(cfg: VilaroConfig, model: string): VilaroConfig {
...
export function applyModelAllowlist(cfg: VilaroConfig, models: string[]): VilaroConfig {
  const defaults = cfg.agents?.defaults;
