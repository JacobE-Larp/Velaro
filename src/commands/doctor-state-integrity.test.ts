...
type EnvSnapshot = {
  HOME?: string;
  VILARO_HOME?: string;
  VILARO_STATE_DIR?: string;
  VILARO_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    VILARO_HOME: process.env.VILARO_HOME,
    VILARO_STATE_DIR: process.env.VILARO_STATE_DIR,
    VILARO_OAUTH_DIR: process.env.VILARO_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: VilaroConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: VilaroConfig) {
...
async function runStateIntegrityText(cfg: VilaroConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "vilaro-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.VILARO_HOME = tempHome;
    process.env.VILARO_STATE_DIR = path.join(tempHome, ".vilaro");
    delete process.env.VILARO_OAUTH_DIR;
    fs.mkdirSync(process.env.VILARO_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when VILARO_OAUTH_DIR is explicitly configured", async () => {
    process.env.VILARO_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: VilaroConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints vilaro-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: VilaroConfig = {};
    ...
    expect(text).toMatch(/vilaro sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/vilaro sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /vilaro sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
