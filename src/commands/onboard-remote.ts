...
function validateGatewayWebSocketUrl(value: string): string | undefined {
  ...
  if (
    !isSecureWebSocketUrl(trimmed, {
      allowPrivateWs: process.env.VILARO_ALLOW_INSECURE_PRIVATE_WS === "1",
    })
  ) {
    return (
      "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel. " +
      "Break-glass: VILARO_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks."
    );
  }
  return undefined;
}
  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        "Docs: https://docs.vilaro.ai/gateway/discovery",
      ].join("\n"),
      "Discovery",
    );
  }
      ...
      if (mode === "direct") {
        ...
        await prompter.note(
          [
            ...
            "Docs: https://docs.vilaro.ai/gateway/remote",
          ].join("\n"),
          "SSH tunnel",
        );
      }
