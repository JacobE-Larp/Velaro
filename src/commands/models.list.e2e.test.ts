...
const ensureVilaroModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveVilaroAgentDir = vi.fn().mockReturnValue("/tmp/vilaro-agent");
const ensureAuthProfileStore = vi.fn().mockReturnValue({ version: 1, profiles: {} });
const listProfilesForProvider = vi.fn().mockReturnValue([]);
const resolveAuthProfileDisplayLabel = vi.fn(({ profileId }: { profileId: string }) => profileId);
const resolveAuthStorePathForDisplay = vi
  .fn()
  .mockReturnValue("/tmp/vilaro-agent/auth-profiles.json");
...
beforeEach(() => {
  ...
  ensureVilaroModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureVilaroModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureVilaroModelsJson).toHaveBeenCalled();
    expect(ensureVilaroModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
