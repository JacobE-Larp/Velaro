...
const SEARCH_TOOL = "https://docs.vilaro.ai/mcp.SearchVelaro";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.vilaro.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('vilaro docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.vilaro.ai/");
      runtime.log(`Search: ${formatCliCommand('vilaro docs "your query"')}`);
    }
    return;
  }
