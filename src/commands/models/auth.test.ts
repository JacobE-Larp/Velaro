...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: VilaroConfig;
  let lastUpdatedConfig: VilaroConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/vilaro/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/vilaro/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/vilaro/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(async (mutator: (cfg: VilaroConfig) => VilaroConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/vilaro/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/vilaro/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `vilaro plugins list --json`.',
    );
  });
