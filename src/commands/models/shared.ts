...
import { type VilaroConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function loadValidConfigOrThrow(): Promise<VilaroConfig> {
...
export async function updateConfig(
  mutator: (cfg: VilaroConfig) => VilaroConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: VilaroConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: VilaroConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("velaro agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
