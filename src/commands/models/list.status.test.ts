...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveVilaroAgentDir: vi.fn().mockReturnValue("/tmp/vilaro-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/vilaro-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/vilaro-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/vilaro-dev/vilaro.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveVilaroAgentDir: mocks.resolveVilaroAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/vilaro-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveVilaroAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/vilaro-dev/vilaro.json");
    expect(payload.auth.storePath).toBe("/tmp/vilaro-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/vilaro-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/vilaro-agent-custom");
