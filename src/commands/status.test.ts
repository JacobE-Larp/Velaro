...
beforeAll(() => {
  envSnapshot = captureEnv(["VILARO_PROFILE"]);
  process.env.VILARO_PROFILE = "isolated";
});
...
vi.mock("../infra/vilaro-root.js", () => ({
  resolveVilaroPackageRoot: vi.fn().mockResolvedValue("/tmp/vilaro"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/vilaro/pnpm-lock.yaml",
      markerPath: "/tmp/vilaro/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.vilaro.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.vilaro.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("vilaro status --all") ||
          line.includes("vilaro --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("VILARO_GATEWAY_TOKEN", "abcd1234", async () => {
