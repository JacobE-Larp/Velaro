...
export function normalizeCompatibilityConfigValues(cfg: VilaroConfig): {
  config: VilaroConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: VilaroConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as VilaroConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as VilaroConfig["browser"],
    };
    changes.push(
