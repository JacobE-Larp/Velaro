...
function resolveAgentName(cfg: VilaroConfig, agentId: string) {
...
function resolveAgentModel(cfg: VilaroConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: VilaroConfig): AgentSummary[] {
