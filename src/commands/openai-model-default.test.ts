...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: VilaroConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: VilaroConfig) => { changed: boolean; next: VilaroConfig };
  defaultModel: string;
  overrideConfig: VilaroConfig;
  alreadyDefaultConfig: VilaroConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: VilaroConfig) => config,
      applyDefaultConfig: (config: VilaroConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: VilaroConfig = { agents: { defaults: {} } };
