...
describe("resolveGatewayAuthTokenForService", () => {
  ...
  it("falls back to VILARO_GATEWAY_TOKEN when SecretRef is unresolved", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
      ...
      {
        VILARO_GATEWAY_TOKEN: "env-fallback-token",
      } as NodeJS.ProcessEnv,
    );
  ...
  it("falls back to VILARO_GATEWAY_TOKEN when SecretRef resolves to empty", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
    ...
    const required = shouldRequireGatewayTokenForInstall(
      {
        ...
        env: {
          vars: {
            VILARO_GATEWAY_PASSWORD: "configured-password", // pragma: allowlist secret
          },
        },
      } as VilaroConfig,
      {} as NodeJS.ProcessEnv,
