...
function createMemorySearchCfg(options: { workspaceDir: string; indexPath: string }): VilaroConfig {
  return {
    agents: {
      defaults: {
...
describe("MemoryIndexManager.readFile", () => {
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-mem-read-"));
