...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "vilaro-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): VilaroConfig {
    const defaults: NonNullable<NonNullable<VilaroConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: VilaroConfig) {
