---
name: node-connect
description: Diagnose Vilaro node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Vilaro is advertising that route, then fix pairing/auth.
...
- whether `vilaro devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `vilaro qr --json`. It uses the same setup-code payload Android scans.

```bash
vilaro config get gateway.mode
vilaro config get gateway.bind
vilaro config get gateway.tailscale.mode
vilaro config get gateway.remote.url
vilaro config get gateway.auth.mode
vilaro config get gateway.auth.allowTailscale
vilaro config get plugins.entries.device-pair.config.publicUrl
vilaro qr --json
vilaro devices list
vilaro nodes status
```

If this Vilaro instance is pointed at a remote gateway, also run:

```bash
vilaro qr --remote --json
```
...
If `vilaro qr --json` says `Gateway is only bound to loopback`:
...
- `vilaro devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run vilaro qr again, rescan, then approve the pending device pairing.`
