---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Vilaro deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Vilaro cron scheduling for periodic checks, or version status checks on a machine running Vilaro (laptop, workstation, Pi, VPS).
---

# Vilaro Host Hardening

## Overview

Assess and harden the host running Vilaro, then align it to a user-defined risk tolerance without breaking access. Use Vilaro security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Vilaro changes the host firewall, SSH, or OS updates; it does not.
...
5. Vilaro gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Vilaro can access sensitive data.
...
### 2) Run Vilaro security audits (read-only)

As part of the default read-only checks, run `vilaro security audit --deep`. Only offer alternatives if the user requests them:

1. `vilaro security audit` (faster, non-probing)
2. `vilaro security audit --json` (structured output)

Offer to apply Vilaro safe defaults (numbered):

1. `vilaro security audit --fix`

Be explicit that `--fix` only tightens Vilaro defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Vilaro version/update status (read-only)

As part of the default read-only checks, run `vilaro update status`.
...
- Credential hygiene notes (location of Vilaro creds, prefer disk encryption)
...
After Vilaro install or first hardening pass, run at least one baseline audit and version check:

- `vilaro security audit`
- `vilaro security audit --deep`
- `vilaro update status`

Ongoing monitoring is recommended. Use the Vilaro cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `vilaro cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `vilaro update status`
...
Before creating, `vilaro cron list` and match on exact `name`. If found, `vilaro cron edit <id> ...`.
If not found, `vilaro cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `vilaro update status` (preferred for source checkouts and channels)
2. `npm view vilaro version` (published npm version)

## Vilaro command accuracy

Use only supported commands and flags:

- `vilaro security audit [--deep] [--fix] [--json]`
- `vilaro status` / `vilaro status --deep`
- `vilaro health --json`
- `vilaro update status`
- `vilaro cron add|list|runs|run`

Do not invent CLI flags or imply Vilaro enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Vilaro compaction:
